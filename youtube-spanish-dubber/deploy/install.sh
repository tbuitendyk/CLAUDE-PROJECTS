...
# === TEMP one-shot: backfill Job 6d1cf4f81841's transcript into the library ===
# Older successful dubs predate transcript retention. This pulls that job's
# stored transcript (the preview `result` rows, or its edited transcript_
# overrides) into the new `transcript`/`title` columns so it shows in the
# library connected to its published video. Idempotent (skips if already set);
# remove after it has run once.
( set +e
  echo "########## [BACKFILL 6d1cf4f81841] ##########"
  "${INSTALL_DIR}/.venv/bin/python" - <<'PY'
import json, sqlite3, subprocess
from datetime import datetime, timezone

JOB = "6d1cf4f81841"
FALLBACK_TITLE = '[Versión Español] Martin Luther estaba equivocado sobre "Trabajos de la ley"... Y cambia todo (Marti'
DB = "/opt/youtube-dubber/data/jobs.sqlite3"
YT = "/opt/youtube-dubber/bin/yt-dlp"
PLUGINS = "/opt/youtube-dubber/yt-dlp-plugins"

con = sqlite3.connect(DB, timeout=30)
con.row_factory = sqlite3.Row
row = con.execute("SELECT * FROM jobs WHERE id=?", (JOB,)).fetchone()
if row is None:
    print("JOB NOT FOUND:", JOB); raise SystemExit
d = dict(row)
print("mode:", d.get("mode"), "| status:", d.get("status"))
print("source_url:", d.get("source_url"))
print("youtube_video_url:", d.get("youtube_video_url"))
print("current title:", d.get("title"))
print("has transcript already:", bool(d.get("transcript")))
print("transcript_overrides bytes:", len(d.get("transcript_overrides") or ""))
print("result bytes:", len(d.get("result") or ""))

if d.get("transcript"):
    print("ALREADY HAS A TRANSCRIPT -- no change."); raise SystemExit
...
if d.get("result"):
    try:
        ...
        if isinstance(res, dict) and isinstance(res.get("rows"), list) and res["rows"]:
            rows = [{"start": r.get("start"), "end": r.get("end"),
                     "original_text": r.get("original_text"),
                     "translated_text": r.get("translated_text")} for r in res["rows"]]
            src = "result.rows (preview)"
    except Exception as e:
        print("result parse error:", e)
if rows is None and d.get("transcript_overrides"):
    try:
        ov = json.loads(d["transcript_overrides"])
        if isinstance(ov, list) and ov:
            rows = [{"start": r.get("start"), "end": r.get("end"),
                     "original_text": None, "translated_text": r.get("text")} for r in ov]
            src = "transcript_overrides (edited lines)"
    except Exception as e:
        print("overrides parse error:", e)

if not rows:
    print("NO STORED TRANSCRIPT DATA (no result rows, no overrides) -- nothing written.")
    raise SystemExit

print("transcript source:", src, "| lines:", len(rows))
print("first line:", rows[0])
print("last line:", rows[-1])

title = FALLBACK_TITLE
url = d.get("youtube_video_url")
if url:
    try:
        out = subprocess.run([YT, "--no-warnings", "--plugin-dirs", PLUGINS,
                              "--skip-download", "--print", "%(title)s", url],
                             capture_output=True, text=True, timeout=120)
        t = (out.stdout or "").strip()
        if t:
            title = t; print("fetched YouTube title:", title)
        else:
            print("title fetch empty; using provided name. stderr:", (out.stderr or "")[-200:])
    except Exception as e:
        print("title fetch failed (%s); using provided name." % e)
else:
    print("no youtube_video_url on the job; using provided name (entry won't link).")

now = datetime.now(timezone.utc).isoformat()
con.execute("UPDATE jobs SET transcript=?, title=?, updated_at=? WHERE id=?",
            (json.dumps(rows), title, now, JOB))
con.commit()
v = con.execute("SELECT title, length(transcript) FROM jobs WHERE id=?", (JOB,)).fetchone()
print("WROTE transcript (%d lines) + title." % len(rows))
print("verify -> title:", v[0], "| transcript bytes:", v[1])
PY
  echo "########## [BACKFILL] end ##########"
) || true
# === END TEMP one-shot ===
