...
- **H4 (3.96.0), 2026-09-08 ~07:15 UTC.** Built as ordered: the Version tab after Compute on Setup, the panel moved whole with its two presses, its sleeping button and its last-verdict lines, the marker beside "planted check:" on every Construct screen opening Setup on Version, Verify keeping everything under the panel. Choices made inside the step and recorded here: the tab re-reads both checks every five seconds while it shows and redraws only when a reading changed (one timer, cleared when the tab changes, in place of Verify's separate watcher); the tab opens with the release and the planted check's state on one line, the words the Construct marker uses; a service that did not answer is said to have not answered rather than drawn as NOT CHECKED; "help entries move with them" became hover text on the two presses and the panel's own paragraphs, because Setup has no Help tab and a help entry for a control on no Construct screen fails the help check; two sentences that pointed at Verify's own screen were reworded. Rendered in a browser against a locally started service and looked at before the suite: the tab, the presses lined up with their state, and the marker's click landing on Setup with Version on. The three new or re-aimed scans passed on their first run. **The whole suite found three more**, in a test file none of the greps for the panel's names had reached: they read the panel's words out of Construct by its status fields and its running line; re-aimed at Setup, with the marker's own poll still read on Construct. One guard. Decision 87.
- **Guards, 2026-09-08 ~07:19 UTC.** Seven guards in one batch after the last commit: six caught first time. The seventh — the filter that keeps only the rows a half-life won — was MISSED: the fabricated chain lets a half-life win every row, so keeping every row read the same as keeping only the winners, and the "nothing improved" refusal was reachable only when nothing improved at all. A hole in the instrument, not in the code. The build test now marks one stored row as the unweighted column's and expects it left out of the count and the set, then marks them all and expects the refusal in words; both guards on that test are caught. Test-only follow-up commit; nothing on the box changes.
- **Where the loop ended, 2026-09-08 ~07:25 UTC.** Done: H0 to H4 — 3.93.0, 3.94.0, 3.95.0, 3.96.0 — each committed and pushed on both branches, none deployed: the owner holds the deploy while a stage 3 sweep runs on the box, and the box serves 3.92.1. Waiting for the owner: the deploy (gated on the box being free), then the served record, the word lists and their commit; under the new release both checks read NOT CHECKED on Setup, under Version, and the owner presses. Discovered along the way and PARKED, not in the loop: (1) the half-life run's floor — 180 days of effective training below which a half-life is refused — is a number in code, printed as GUESSED when it refuses; RULE FIVE would make it the owner's control; (2) Setup's tabs have no closed word list, RULE ONE-A naming Construct screens only, so the Version tab's words are on no list; whether Setup gets lists is the owner's call.
