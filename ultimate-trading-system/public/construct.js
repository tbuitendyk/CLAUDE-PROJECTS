...
function vTunedNoteHtml(t) {
  ...
  const body = t.why
    ...
    : `each is read above at its money under them, worked out off its captured trades${cap} in the record's own dollars, $${Number(t.clipUsd) || 0} a trade; its money without them is in the table below${(t.notCaptured || []).length ? ` · ${t.notCaptured.length} with a tuning but no captured trades on this window, read plain` : ''}`;
...
function vSurvivorsTableHtml(b, stretch) {
  ...
  return `<div class="scrollx" style="max-height:24rem;overflow-y:auto"><table><thead><tr>
    ...
    ${tunedOn ? `<th title="the stop and the sizing frozen on this survivor when this set was written; the money column reads it under them">tunings</th>
    <th title="its money on the ${stretchPlain(stretch)} window with no tuning, as the record priced it, in the same dollars">without them $</th>
  ...
  </tr></thead><tbody>${rows.map((r) => { const p = pricedBy.get(r.label) || null; const x = tunedBy.get(r.label) || null; return `<tr><td>${esc(r.label)}</td><td class="${(r.money || 0) >= 0 ? 'pos' : 'neg'}">${money(r.money)}</td><td>${r.trades == null ? '—' : r.trades}</td><td>${money(r.vsLong)}</td><td>${vOwnCell(ownBy.get(r.label))}</td><td>${r.storedBeat == null ? '—' : r.storedBeat}</td><td>${r.storedPairs == null ? '—' : r.storedPairs}</td><td>${vFix(r.storedLead)}</td><td>${r.beats} of ${r.copiesKept}</td><td class="${r.pass ? 'pos' : 'neg'}">${r.pass ? 'PASS' : 'FAIL'}</td>${tunedOn ? (x ? `<td>${vTuningsWords(x)}</td><td class="${(x.plainUsd || 0) >= 0 ? 'pos' : 'neg'}">${money(x.plainUsd)}</td><td>${x.stopped ? `${x.stopped} of ${x.priced}` : '—'}</td>` : '<td class="muted">none</td><td>—</td><td>—</td>') : ''}${priced ? `<td>${p && p.stops != null ? p.stops : '—'}</td><td>${money(p && p.ride ? p.ride.maxDrawdown : null)}</td><td>${money(p && p.ride ? p.ride.worstTrade : null)}</td>` : ''}</tr>`; }).join('')}</tbody></table></div>
