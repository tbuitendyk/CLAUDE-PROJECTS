...
// column heading says whose number each cell is. Drawn on Held and on Reserve
// alike, the money column named after the stretch as two whole phrases (3.147.0).
function readPanel(title, h, c, stretch) {
  ...
  return `<p class="note"><b>${esc(title)}</b></p>
    <div class="scrollx"><table><thead><tr>
      ...
      <th title="the mean money those settings made on this window, one figure a setting">${stretch === 'reserve' ? '<span>reserve $ a setting</span>' : '<span>held-back $ a setting</span>'}</th>
...
// order it reads in is a claim about how the work goes. The judgement used to
// sit fifth, which said it happens before History and Tune have finished
...
// immediately before Greenlight: everything that picks, then the two tabs
// that judge -- Held, then Reserve, the same judgement on the next stretch
// (3.147.0, VERIFY-DESIGN.md Part 9) -- then the decision to trade. Help is
// not part of the flow and stays at the end.
//
...
// ---- Held and Reserve: the verdict on a Stage 4 record set, on one stretch each --------
//
// (3.86.0, VERIFY-DESIGN.md; two tabs off one renderer since 3.147.0, Part 9.)
// ONE RENDERER, THE STRETCH AS ITS ONLY PARAMETER. drawHeld and drawReserve
// are one line each; everything they draw is drawJudge(stretch) and the
// helpers below, so the two tabs cannot drift apart (RULE TWO's shape). Every
// place the code asks which stretch it is on is a deliberate difference: the
// words for the window, which rules the box lists, and the confirm before a
// press that prices.
//
// A PRESS MAKES A SET. Reading a rule on a stretch writes a held set or a
// reserve set of the rule, named after it, numbered from the second press;
// the sets already made are drawn under the press, newest first, the newest
// in full. The three readings under them belong to the rule, per stretch.
const JUDGE_SET_KEY = { held: 'cx-held-set', reserve: 'cx-reserve-set' };
// THE STRETCH'S OWN WORDS, WHOLE. A label that differs by stretch is written as
// two whole phrases, each wrapped in a tag, because the word list's walk keeps
// a quoted string as page text only when it carries a tag and reads the text
// between tags as one name -- so "Read the rule on the held-back window" lands
// on the list whole, and so does its Reserve twin, on both tabs. Both ARE on
// both screens: Reserve names the held set it stands on, Held says when a
// layout keeps no reserve. stretchWindow() is the one noun phrase every
// sentence here shares; hover text and confirm boxes take stretchPlain(),
// which carries no tag.
function stretchWindow(stretch) { return stretch === 'reserve' ? '<span>reserve window</span>' : '<span>held-back window</span>'; }
function stretchPlain(stretch) { return stretch === 'reserve' ? 'reserve' : 'held-back'; }
function tabWord(stretch) { return stretch === 'reserve' ? 'Reserve' : 'Held'; }
function vRememberedSet(list, stretch) {
  let want = null;
  try { want = localStorage.getItem(JUDGE_SET_KEY[stretch]); } catch (_) { want = null; }
...
// they are what a press makes, and they are drawn under it.
function vRulesFor(sets, stretch) {
  const rules = (sets || []).filter((x) => (x.kind || 'funnel') === 'funnel');
...
function vFootingHtml(d) {
  ...
  if (!f) return '';
  const r = f.releases || {};
  return `<p class="note"><b>Footing:</b> ${f.halfLife
    ? (f.ok ? `every one of its ${Number(f.had).toLocaleString()} records is still on the board` : `<b class="neg">${esc(String(f.why || ''))}</b>`)
    : (f.same && !f.gone
...
function vLinesHtml(b) {
  const a = b.lineA || null;
  const l = b.lineB || null;
  if (!a && !l) return '';
...
// "x.known" and "x.clears" on the list as though the owner could see them.
function vOwnCell(x) {
...
function vSurvivorsTableHtml(b, stretch) {
  ...
  const pricedBy = new Map((b.priced || []).map((x) => [x.label, x]));
  const priced = pricedBy.size > 0;
  return `<div class="scrollx" style="max-height:24rem;overflow-y:auto"><table><thead><tr>
    ...
    <th title="dollars on the ${stretchPlain(stretch)} window, the stamped look">${stretch === 'reserve' ? '<span>avg reserve $</span>' : '<span>avg held-back $</span>'}</th>
    <th title="positions taken on the ${stretchPlain(stretch)} window">trades</th>
    <th title="its ${stretchPlain(stretch)} dollars against simply holding the coin over the same days">vs always long $</th>
    ...
    <th title="as stored on the record: of its own null copies, raw dollars over every deal, how many its ${stretchPlain(stretch)} money beat">beat its own null set</th>
  ...
  </tr></thead><tbody>${rows.map((r) => { const p = pricedBy.get(r.label) || null; const x = tunedBy.get(r.label) || null; return `<tr><td>${esc(r.label)}</td><td class="${(r.money || 0) >= 0 ? 'pos' : 'neg'}">${money(r.money)}</td><td>${r.trades == null ? '—' : r.trades}</td><td>${money(r.vsLong)}</td><td>${vOwnCell(ownBy.get(r.label))}</td><td>${r.storedBeat == null ? '—' : r.storedBeat}</td><td>${r.storedPairs == null ? '—' : r.storedPairs}</td><td>${vFix(r.storedLead)}</td><td>${r.beats} of ${r.copiesKept}</td><td class="${r.pass ? 'pos' : 'neg'}">${r.pass ? 'PASS' : 'FAIL'}</td>${tunedOn ? (x ? `<td>${vTuningsWords(x)}</td><td class="${(x.plainUsd || 0) >= 0 ? 'pos' : 'neg'}">${money(x.plainUsd)}</td><td>${x.sizing && x.sizing.on ? `<span class="${(x.tunedUsd || 0) >= 0 ? 'pos' : 'neg'}">${money(x.tunedUsd)}</span>` : '—'}</td><td>${x.clipsPerTrade == null || !(x.sizing && x.sizing.on) ? '—' : vFix(x.clipsPerTrade)}</td><td>${x.stopped ? `${x.stopped} of ${x.priced}` : '—'}</td>` : '<td class="muted">none</td><td>—</td><td>—</td><td>—</td><td>—</td>') : ''}${priced ? `<td>${p && p.stops != null ? p.stops : '—'}</td><td>${money(p && p.ride ? p.ride.maxDrawdown : null)}</td><td>${money(p && p.ride ? p.ride.worstTrade : null)}</td>` : ''}</tr>`; }).join('')}</tbody></table></div>
...
// both tabs. The newest is open; earlier ones fold, so a rule read three times
// is three sets on the page and one of them in full.
function vBlockHtml(set, isNewest, stretch) {
  const b = set.block || {};
  const v = b.verdict || {};
  const h = b.read || {};
  ...
  const body = `<p class="note">${esc(v.sentence || '')}</p>
    ...
    <p class="note">sanity: ${sn.known ? `${sn.over === 'survivors' ? "over the survivors' copies only, " : ''}${vPct((sn.board || {}).losing)} of ${Number((sn.board || {}).figures || 0).toLocaleString()} ${stretch === 'reserve' ? '<span>scrambled reserve figures lose money</span>' : '<span>scrambled held-back figures lose money</span>'}${sn.over === 'survivors' ? '' : ` (among the survivors ${vPct((sn.survivors || {}).losing)})`}, threshold ${sn.threshold}% - ${sn.ok
    ...
    ? `${b.others.positive} of ${b.others.of} other units positive on the ${stretchWindow(stretch)}; ${b.others.clearBar} clear the bar${b.others.keepsNothing ? ` · ${b.others.keepsNothing} keep nothing` : ''}${b.others.mark ? ` · <b class="warn">${esc(b.others.mark)}</b>` : ''} <span class="muted">(read ${esc(String(b.others.at || '').slice(0, 16))}, information only)</span>`
    : 'not read when this was stamped'}</p>
    ${vLinesHtml(b)}
    ${(b.missing || []).length ? `<p class="note"><b class="warn">${b.missing.length} survivor(s) were not priced</b> - they are not in the stage 3 set's block on this unit</p>` : ''}
  ...
  return `<div class="panel" style="margin-top:.5rem">
    ...
    ${isNewest ? body : `<details><summary>the reading on this set</summary>${body}</details>`}
  </div>`;
}
function vSetPanelHtml(list, chosen, d, stretch) {
  const sets = d ? (d.sets || []) : [];       // newest first, as the engine lists them
  return `<div class="panel">
    <h3 style="margin-top:0">${stretch === 'reserve' ? '<span>The verdict on the reserve window</span>' : '<span>The verdict on the held-back window</span>'}</h3>
    <p class="note">A rule can be checked against scrambled data and a single row cannot, so this reads the rule as a
      whole: what its survivors made on the ${stretchWindow(stretch)}, each against the four simpler things the Funnel prints
      ...
      is the stamped look, and every look is counted. Each press writes a ${stretch === 'reserve' ? '<span>reserve set</span>' : '<span>held set</span>'} of the rule, the rule and
      its Tune choices frozen at that moment with its one verdict, and that set is what Greenlight reads.${stretch === 'reserve'
...
// where this rule's reserve window begins, in its record's own terms
function vWindowWords(w) {
  if (!w) return '';
  if (!w.keeps) return `<span class="muted">${esc(String(w.why || ''))}</span>`;
  if (!w.intact) return `reserve window <b class="neg">not readable</b> - ${esc(String(w.why || ''))}`;
  return `reserve window from ${vDay(w.fromTs)} onward: the sealed reserve, cut away before anything trained${w.chunks != null ? `, ${w.chunks} chunks at the seal` : ''}`;
}
...
// THE RULE ON THE OTHER UNITS (V6, 3.88.0): the newest reading in full, earlier
// ones one line each; every press appends and none is overwritten.
...
function vDroppedHtml(d, stretch) {
  const list = (d.readings || {}).dropped || [];
  const o = list[0] || null;
  ...
  return `<div class="panel" style="margin-top:.5rem">
    <h4 style="margin:0 0 .3rem">${stretch === 'reserve' ? '<span>What the rule dropped, reserve window</span>' : '<span>What the rule dropped, held-back window</span>'}</h4>
    <p class="note">The settings the rule did NOT keep, read on the same ${stretchWindow(stretch)} as the survivors. A count of
      ...
      counted look at the ${stretchWindow(stretch)}. Information only, never a gate.</p>
    <div class="row" style="align-items:center">
      ...
      <button id="vDropped" class="pri" ${d.droppedRefused ? 'disabled' : ''} title="reads the ${stretchPlain(stretch)} money already stored for the settings the rule dropped, beside the same figures for the survivors, and appends the reading to the rule. Never a pass or fail on the set.">Read what the rule dropped</button>
    ...
    ${o ? `<p class="note">${esc(o.sentence || '')} <span class="muted">read ${esc(String(o.at || '').slice(0, 16))} under release ${esc(o.release || '?')}</span></p>
      <div class="scrollx"><table><thead><tr>
        ...
        <th title="how many of them made money on the ${stretchPlain(stretch)} window, and what share of those with a figure">positive</th>
        ...
        <th title="the mean ${stretchPlain(stretch)} money over this side">${stretch === 'reserve' ? '<span>avg reserve $</span>' : '<span>avg held-back $</span>'}</th>
        <th title="the middle ${stretchPlain(stretch)} figure of this side, which a few very large ones cannot move">${stretch === 'reserve' ? '<span>median reserve $</span>' : '<span>median held-back $</span>'}</th>
        <th title="settings on this side with no ${stretchPlain(stretch)} figure at all, counted and never dropped">no figure</th>
      </tr></thead><tbody>
...
function vOthersHtml(d, stretch) {
  const list = (d.readings || {}).others || [];
  const o = list[0] || null;
  const earlier = list.slice(1);
  return `<div class="panel" style="margin-top:.5rem">
    <h4 style="margin:0 0 .3rem">${stretch === 'reserve' ? '<span>The rule on the other units, reserve window</span>' : '<span>The rule on the other units, held-back window</span>'}</h4>
    <p class="note">The same rule on every other coin-and-shape unit of the stage 3 set this was cut from, each read on its
      own ${stretchWindow(stretch)} against its own scrambled copies at the bar declared above. Two counts, information only,
    ...
    ${o ? `<p class="note"><b>${o.positive} of ${o.of} other units positive; ${o.clearBar} clear the bar</b>${o.keepsNothing ? ` · ${o.keepsNothing} keep nothing` : ''}${o.notPriced ? ` · <b class="warn">${o.notPriced} not priced on the reserve window yet</b>` : ''}${o.mark ? ` · <b class="warn">${esc(o.mark)}</b>` : ''} <span class="muted">read ${esc(String(o.at || '').slice(0, 16))} under release ${esc(o.release || '?')} · bar ${(o.rules || {}).barPct}% (${esc(((o.rules || {}).tags || {}).bar || '')})</span></p>
      <div class="scrollx" style="max-height:20rem;overflow-y:auto"><table><thead><tr>
        ...
        <th title="the mean ${stretchPlain(stretch)} money of the settings the rule keeps there">${stretch === 'reserve' ? '<span>avg reserve $</span>' : '<span>avg held-back $</span>'}</th>
...
// THE RIDE (V7, 3.88.0): per survivor, the stretch's half beside the test half,
// stamped with the release that computed it.
function vRideHtml(d, stretch) {
  const list = (d.readings || {}).ride || [];
  const r = list[0] || null;
  const earlier = list.slice(1);
  const thirds = (h) => (h && Array.isArray(h.pnlThirds) && h.pnlThirds.length ? h.pnlThirds.map((v) => money(v)).join(' / ') : '—');
  return `<div class="panel" style="margin-top:.5rem">
    <h4 style="margin:0 0 .3rem">${stretch === 'reserve' ? '<span>The ride on the reserve window</span>' : '<span>The ride on the held-back window</span>'}</h4>
    <p class="note">What the ${stretchWindow(stretch)} looked like from inside, per survivor: the largest drawdown, the worst
      ...
      minutes. Information only, never a gate, and every press is a stamped look at the ${stretchWindow(stretch)}.</p>
    <div class="row" style="align-items:flex-end">
      <button id="vRide" class="pri" ${d.rideRefused ? 'disabled' : ''} title="prices this rule's survivors on this unit again and keeps the ${stretchPlain(stretch)} half of what the pricing works out, beside the test half, with the release that computed it. A stamped look; never a pass or fail.">${stretch === 'reserve' ? '<span>Work out the reserve ride</span>' : '<span>Work out the held-back ride</span>'}</button>
      <span id="vRideMsg" class="note">${d.rideRefused ? `<b class="warn">refused:</b> ${esc(d.rideRefused)}` : ''}</span></div>
    ${r ? `<p class="note"><span class="muted">worked out ${esc(String(r.at || '').slice(0, 16))} under release ${esc(r.release || '?')}</span> · ${(r.rows || []).length} of ${r.settings} survivors${(r.missing || []).length ? ` · <b class="warn">${r.missing.length} not priced</b>` : ''}${(r.failures || []).length ? ` · <b class="warn">${r.failures.length} unit failure(s)</b>` : ''}${r.forecasts ? ` · priced with ${esc(r.forecasts)}` : ''}</p>
      <div class="scrollx" style="max-height:24rem;overflow-y:auto"><table><thead><tr>
        <th title="the setting, by the name the board gives it">setting</th>
        <th title="dollars on the ${stretchPlain(stretch)} window, as priced by this pass">${stretch === 'reserve' ? '<span>reserve $</span>' : '<span>held-back $</span>'}</th>
        <th title="positions taken on the ${stretchPlain(stretch)} window">trades</th>
        <th title="the deepest fall from a high point of the running money on the ${stretchPlain(stretch)} window, in dollars">largest drawdown $</th>
        <th title="the single worst trade on the ${stretchPlain(stretch)} window">worst trade $</th>
        <th title="the single best trade on the ${stretchPlain(stretch)} window">best trade $</th>
        <th title="trades that closed with a gain on the ${stretchPlain(stretch)} window">trades won</th>
        <th title="trades closed by the stop on the ${stretchPlain(stretch)} window">stopped out</th>
        <th title="money per trade before fees on the ${stretchPlain(stretch)} window">gross per trade $</th>
        <th title="the ${stretchPlain(stretch)} window's money in three equal parts, first to last">money by third</th>
    ...
    : '<p class="note">Not worked out on this rule yet.</p>'}
  </div>`;
}
// the window layout by the words the Sweep screen offers it under; History says it too
function layoutWords(layout) {
  return layout === 'reserve61' ? '61/13/13/13 (sealed exam)' : layout === 'split70' ? '70/15/15' : 'unrecorded';
}
// ONE RENDERER FOR BOTH TABS. The tab's own function is one line; the stretch
// is the only thing that differs, and the follow loops stop when the tab is
...
async function drawJudge(stretch) {
  ...
  // this one is what both tabs' one-line renderers hand the whole screen to.
  $('#view').innerHTML = `<div class="judge" data-stretch="${stretch}">${vSetPanelHtml(sets, chosen, d, stretch)}</div>`;
  const where = `The Stage 4 record set box on ${tabWord(stretch)} lists what was stamped - pick the rule there.`;
  const sel = $('#vSet');
  if (sel) sel.onchange = () => {
  ...
  if (btn && chosen && d && !d.refused) btn.onclick = async () => {
    ...
    if (d.prices) {
      const looks = (d.sets || []).length;
      const msg = stretch === 'reserve'
        ? (looks === 0
          ? 'Read the rule on the reserve window?\n\nThis prices its survivors on the reserve window and writes a reserve set. It is the FIRST look at data nothing in this system has seen. After it, that is no longer true.'
          : `Read the rule on the reserve window again?\n\nThis window has already been read ${looks} time(s) on this rule. The set it writes will say so: the floor it prints is the best case, and the real strength is weaker by an amount nothing here can measure.`)
  ...
  if (ob && chosen && d && !d.othersRefused) ob.onclick = async () => {
    ...
    const started = await tryPost(`api/funnel/set/${encodeURIComponent(chosen)}/others`, { stretch, barPct: vTyped('#vBarPct') }, where);
    if (!started) { ob.disabled = false; $('#vOthersMsg').textContent = ''; return; }
    vOthersFollow(chosen, started.token, stretch);
  };
  const db = $('#vDropped');
  if (db && chosen && d && !d.droppedRefused) db.onclick = async () => {
    db.disabled = true;
    $('#vDroppedMsg').textContent = 'reading…';
    const got = await tryPost(`api/funnel/set/${encodeURIComponent(chosen)}/dropped`, { stretch, sample: vTyped('#vDroppedN') }, where);
    db.disabled = false;
    $('#vDroppedMsg').textContent = '';
    if (got) drawJudge(stretch);
  };
  const rb = $('#vRide');
  if (rb && chosen && d && !d.rideRefused) rb.onclick = async () => {
    if (!confirm(`Work out the ${stretchPlain(stretch)} ride?\n\nPrices this rule's survivors on this unit again and keeps the ${stretchPlain(stretch)} half beside the test half. Minutes. It is a stamped look at the ${stretchPlain(stretch)} window, counted on every later verdict.`)) return;
    rb.disabled = true;
    $('#vRideMsg').textContent = 'starting…';
    const started = await tryPost(`api/funnel/set/${encodeURIComponent(chosen)}/ride`, { stretch }, where);
...
// ---- History (the retrain run) ---------------------------------------------------
// HISTORY COMES AFTER THE FUNNEL AND BEFORE TUNE AND VERIFY (3.142.0, owner
...
// grade is a reserve set on Reserve, under the held set it stands on.
...
function tnTargetRowHtml(cand, pick, wins) {
  ...
  <div class="row" style="margin-bottom:.4rem;align-items:flex-end">
    <span class="note" title="which of the three windows' entries the scans read. The training and test windows were read to choose the rule, so reading them again is not a look; the held-back window was sealed until Held, so every scan that reads it is a counted look.">windows the scans read</span>
...
async function drawSweep() {
  ...
  <div class="panel">
    ...
    <div class="row" style="margin-top:.5rem;align-items:flex-end">
      ...
      <div style="display:flex;align-items:flex-end;gap:.45rem">
        <label class="f" title="WHAT IS WEIGHED when the members are polled. count is how many say the same thing — the plain head count. conviction is how hard they lean, added up, so six that are certain outweigh six that barely lean. voices is a head count in which members that almost always call the same way as each other share one vote between them, so a crowd of near-copies cannot outvote a real disagreement. families is how many different KINDS of evidence agree — the members read four different slices of the numbers, and this asks that several slices line up rather than several members. trained is the way stages 1 and 2 added the members up when they scored these units: every lean added together and the winning side taken, whatever the margin and whoever the head count would have picked. It is the only choice here that reads no bar at all, because those stages had none — so quorum bar and share are left out of its name and off its records, and it is one setting however many of either are being priced. field is the field alone: on every day the field named under The field speaks, its sign is the call and the members are not read, so the bars under The field are the whole trigger; quorum bar and share are left out of its name and off its records, and it is one setting however many of either are being priced. With no field named it is refused, and permute leaves it out. This box is only half the quorum: quorum bar decides how much of it is enough.">quorum by<select id="swAgreeRule">${vocabOptions('agreeRule', 'count')}</select></label>
        <label class="c" title="price every quorum by choice as its own setting — field only where a field is named under The field."><input type="checkbox" id="swPermAgreeRule"> permute</label>
...
const B_HELD_BACK_WORDS_3B = { share: 'beat its own null set', pairs: 'comparisons', money: 'avg held-back', trades: 'avg held-back trades', vslong: 'avg vs always-long' };
  ...
  if (!bPut(mount, `${incomplete}<div class="panel">
    ...
    <div class="scrollx"><table style="border-collapse:collapse">
      <thead><tr style="text-align:left;border-bottom:1px solid var(--line)">
        ...
        <th ${bth} title="when this unit was trained with weigh each trade by the money it was worth ticked on Sweep: how many ordinary trades the biggest one would have counted for if nothing had held it down, and after the slash how many were held at the ceiling — the number in the box called the most one trade may count for on Sweep, stage 1. It is what says whether that box is doing anything, and how far it is toning things down. Blank when the run was trained by direction only, and on any run that finished before this began being recorded.">biggest before the ceiling${bSortBtn(doc, 'biggestBeforeCap', 'desc')}</th></tr></thead>
      <tbody>${rows.map((r) => `<tr>
  ...
  if (!bPut(mount, `${incomplete}<div class="panel">
    ...
    <div class="scrollx"><table style="border-collapse:collapse">
      <thead><tr style="text-align:left;border-bottom:1px solid var(--line)">
        ...
        <th ${bth} title="average entries per coin on the test window — how often this setting actually trades. Read on the test window, so it stays when the held-back window is hidden.">avg test trades${bRankSortBtn(doc, 'testTrades', 'desc')}</th>
      ...
      <tbody>${rr.map((r, i) => `<tr>
        ...
        <td ${btd}>${r.testTrades == null ? '—' : r.testTrades.toFixed(1)}</td>
    ...
    ['minTestTrades', 'avg test trades at least', 'num', 'hides rows whose records traded fewer times than this on the test window, on average. Empty hides nothing.'],
    ...(bHeldBack ? [
    ['minHold', 'avg held-back at least, $', 'num', 'hides rows whose average held-back money is below this. Empty hides nothing.'],
    ['minTrades', 'avg held-back trades at least', 'num', 'hides rows with fewer average held-back entries than this. Empty hides nothing.'],
    ...
    <div class="scrollx"><table style="border-collapse:collapse"><thead><tr data-bcoinhead style="text-align:left;border-bottom:1px solid var(--line)">
        ...
        <th ${bth} title="average entries per record on the test window — how often this coin's records actually trade. Read on the test window, so it stays when the held-back window is hidden.">avg test trades${bCoinSortBtn(view, 'testtrades', '↓')}</th>
    ...
    return `<tr data-bkey="${esc(k)}">
        ...
        <td ${btd}>${bMoney(r.avgTest)}</td>
        <td ${btd}>${r.avgTestTrades == null ? '—' : r.avgTestTrades.toFixed(1)}</td>
        ${bHeldBack ? `<td ${btd}>${bMoney(r.avgHold)}</td>
        <td ${btd}>${r.avgTrades == null ? '—' : r.avgTrades.toFixed(1)}</td>
        <td ${btd}>${bMoney(r.avgVsLong)}</td>` : ''}
        <td ${btd}>${r.avgAgreed == null ? '<span class="muted">—</span>' : `${r.avgAgreed.toFixed(1)}%`}</td>
...
drawHeld = ((fn) => async (...a) => { holdScrollMemory(); const r = await fn(...a); hoverFromHelp('held'); return r; })(drawHeld);
drawReserve = ((fn) => async (...a) => { holdScrollMemory(); const r = await fn(...a); hoverFromHelp('reserve'); return r; })(drawReserve);
...
async function fDrawView() {
  ...
  ${!open ? `<div class="panel">${fHomeNote()}</div>`
...
// the count runs over every coin and shape of the set (3.132.0); a set of more than one says so, and which one the pass is on (3.226.0)
const fAcrossWords = (units, onUnit) => (onUnit && Number(onUnit.of) > 1 ? ` on coin and shape ${Number(onUnit.at || 0).toLocaleString()} of ${Number(onUnit.of).toLocaleString()}` : (Number(units) > 1 ? ` across ${Number(units).toLocaleString()} coins and shapes` : ''));
...
function fRichLine(d) {
  ...
  if (fRichGoing(d)) {
    return (run.of ? `working them out — ${Number(run.done || 0).toLocaleString()} of ${Number(run.of).toLocaleString()} settings${fAcrossWords(run.units, run.onUnit)}`
...
function fTitle(d, st, name, away, open) {
  ...
  // PUT AWAY IS DEAD AT HOME. It has nothing to act on with nothing under this
  // row, and a control that looks live and is not is the fault
  // `Delete Stage 4 record set…` beside it already avoids.
  //
  // `Go to Funnel home` WAS HERE AND IS GONE (3.109.0, owner order
  // 2026-09-11): "So there's no reason to have a button called go to funnel
  // home anywhere ... you changed it to a put away button, which essentially
  // does what that one was supposed to do, and you broke the one that we
  // wanted. So now just get rid of the button Go to funnel home."
  //
  // Put away is the control: it brings this row's section and the one above it
  // back, keeps the walk underneath, and `Open` returns to it. Home is still
  // where the Funnel lands on a set with nothing opened yet.
...
// screen that failed to draw, so Home says it is Home and names the ways out
// of it -- all of them controls on this screen, in the words they carry.
//
// A FUNCTION, NOT A CONSTANT, and that is not a style choice: the word list is
// generated by following the helpers a screen draws with, and a sentence
// hidden behind a plain string is on the owner's screen and on no list, which
// RULE ONE-A says is worse than no list at all.
function fHomeNote() {
  return `<p class="note">Nothing is open. Press <b>Walk this one</b> on a row above to start a walk on that coin and shape,
...
function fCutHead(cd, st, d) {
  ...
  return `<div class="row" style="align-items:flex-end">
    ...
    <p class="note"><b>The rules below were built on test money</b> - the steps read the test window and nothing
      ...
      survive. It is here because <b>History</b>, <b>Tune</b>, <b>Held</b>, <b>Reserve</b> and <b>Greenlight</b> are where a survivor
...
function fCutTable(cd, st) {
  ...
  return `<p class="note">Every one of these ${Number(cd.total).toLocaleString()} settings has the same
    ...
    <div class="row">
      <label class="c" title="the held-back window is priced at stage 3 and kept for Held. Off, which is how this screen always opens, the held-back row under each setting and the sorts on it are not drawn, and a saved sort on one of them is set aside. On, they are drawn — and that is written on this Stage 4 record set as one dated look, which Held counts the way it counts a look on Boards."><input type="checkbox" id="fHeldBack" ${hb ? 'checked' : ''}> show the held-back window</label>
...
async function fRichWatch(st) {
  ...
  try {
    for (;;) {
      ...
      fRebuildSay((p.of ? `working them out — ${Number(p.done || 0).toLocaleString()} of ${Number(p.of).toLocaleString()} settings${fAcrossWords(p.units, p.onUnit)}`
...
drawHeld = waitWrap(drawHeld);
drawReserve = waitWrap(drawReserve);
...
//
// THE TYPED VALUE LIVES HERE, NOT IN THE MARKUP, because this panel redraws
// itself while a reading runs. Rebuilt from the markup on every redraw, the
...
//
// Five bars per coin, one per chunk shape. One coloured unit is one decision,
// the same rows Sweep trains on: green where the window rose, red where it
// fell, black where it moved too little either way and would sit out. Above
// each bar the 70/15/15 division; below it the 61/13/13/13 (sealed exam)
// division. Beside each bar, the counts per part. Three controls and nothing
// else (owner: "forget about putting a bunch of controls on it").
//
// THIS SCREEN REPORTS AND NEVER REFUSES. Owner, 2026-09-12: "We're not even
// blocking coins with this anyways. We're only reporting." No cut-offs, no
...
const C_COLOUR = { r: '#1a9c3a', f: '#d0312d', s: '#111111' };
const C_TRACK = '#dfe4ea';
const C_WORD = { r: 'rising', f: 'falling', s: 'sit out' };
const cNum = (v, d = 2) => (v == null ? '—' : Number(v).toFixed(d));
...
const cLayoutLabel = (key) => {
  ...
  return o ? o.label : String(key);
};
// THE BARS ARE PAINTED AT THE PANEL'S WIDTH, so a window that changes width
// repaints them; nothing is fetched again.
window.addEventListener('resize', () => { if (tab === 'coins' && cLastRecs.length) cPaintBars(cLastRecs); });
// THE EVERY-TWO-SECONDS ASK REDRAWS QUIETLY, the same way Boards' does. The
// full-screen wait box flashing twice a second is not information, and while
// it is up it swallows clicks -- including the one on Stop.
function cPollRedraw() {
  waitSilent = true;
  try { holdScrollMemory(); return drawCoins().then(() => holdScrollMemory()); } finally { waitSilent = false; }
}
// WHILE A READING RUNS, ONLY THE STATUS IS ASKED FOR every two seconds, and the
// bars are redrawn when a coin finishes. The records reply carries every
// decision of every bar on the screen; fetching it twice a second to learn
// that nothing has changed is not information.
async function cPollStatus() {
  const st = await apiOr('api/coins/run', null);
  if (tab !== 'coins') return;
  if (!st || !st.running || st.done !== cLastDone) { cPollRedraw(); return; }
  const out = $('#cOut');
  if (out) out.innerHTML = cStatusLine(st, true);
  coinsPoll = setTimeout(cPollStatus, 2000);
}
function cStatusLine(st, anyRead) {
...
function cSortRows(rows, list, OF, tie, DEPTH) {
  ...
  return rows.slice().sort((x, y) => {
    for (const s of sorts) {
      ...
      // THE SECOND KEY FOR A SHARE, always more evidence first whichever way
      // the arrow points. Which columns are shares, and what counts as
      // evidence behind them, is each table's own business -- the sorter is
      // handed a map and does not guess it from a column's name (3.165.0; it
      // used to test for the one key it knew about, which left every share
      // column added after it without a second key).
      const deep = DEPTH && DEPTH[s.key];
      if (deep) { const d = deep(y) - deep(x); if (d) return d; }
    }
    return tie(x, y);
  });
}
const cWalkTie = (x, y) => String(`${x.coin}${x.geometry}${x.lookback}${x.band}`).localeCompare(`${y.coin}${y.geometry}${y.lookback}${y.band}`);
function cWalkSorted(rows) { return cSortRows(rows, cState.wSorts, C_WALK_OF, cWalkTie, C_WALK_DEPTH); }
...
const C_SPLIT_OF = {
  ...
  latePaid: (p) => (p.lateWindows ? p.lateWindowsPaid / p.lateWindows : null),
  lateWorst: (p) => p.lateWorst,
...
// column the owner already sorts by. Reported, not changed.
const C_SPLIT_DEPTH = { latePaid: (p) => p.lateWindows };
const cSplitTie = (x, y) => String(`${x.coin}${x.geometry}`).localeCompare(`${y.coin}${y.geometry}`);
function cSplitSorted(pairs) { return cSortRows(pairs, cState.sSorts, C_SPLIT_OF, cSplitTie, C_SPLIT_DEPTH); }
...
function cWalkList(matches) {
  ...
  // ONE ROW PER PAIR, NOT EVERY ROW OF EVERY PAIR (3.193.0, owner order: "I've
  // got 24 units with their SPECIFIC settings on the [Choose early, read late]
  // view -- that's how many rows I want to see on the table above when the
  // filter is applied! NOT 37,488 rows which is entirely USELESS").
  //
  // The tick used to keep every row belonging to a shown pair. On the owner's
  // set that is a walk's worth of rows per pair -- 37,488 of them -- which is
  // not a review of anything. What a shown pair MEANS on this table is its one
  // row at whole look-back and whole band: the setting to tune with, and the
  // row Promote every row shown sends. So that is the row this keeps, and the
  // keys are built by cSplitWholeRows, which the press builds its keys from
  // too, so the table and the press can never name different rows.
  //
  // With no reading taken it hides nothing, because a tick that empties a
  // fifteen-thousand-row table because a button below has not been pressed is
  // the 3.164.1 fault again.
...
// AND HOW MANY WINDOWS ACTUALLY PAID (3.165.0, owner: "i'm trying to get a fix
// on good numbers on as many windows as possible without a large spread ... is
// there a better metric?").
//
// per trade per spread has two faults and this column answers both. It reads
// only the two EXTREME windows and throws away everything between them, and it
// does not care HOW MANY windows there are -- three and fifteen score alike,
// which is the fault windows up already had and had fixed. Counting the
// windows that cleared the round trip cannot be carried by one spectacular
// window, and more windows beats fewer at the same share.
//
// STRICTLY ABOVE, never equal: a window that made back exactly what it cost to
// trade paid nothing, and counting it says otherwise.
//
...
const C_WALK_F_KEYS = ['bothOnly', 'wholeOnly', 'coin', 'shape', 'back', 'band', 'minTrades', 'minPer', 'minWindows', 'minUp', 'minPaid',
...
function cWalkFilterRow() {
  ...
  return `<div class="filters three">
    ...
    <label class="c frow" title="keep ONE ROW for each coin and chunk shape Choose early, read late is showing &mdash; the row at its whole look-back and whole band, which is the setting to tune with and the row Promote every row shown sends. 24 pairs down there means 24 rows up here, not every row those pairs have. Press Promote every row shown and it comes on by itself. With no reading taken yet it hides nothing, the same as an empty box."><input id="wf_wholeOnly" type="checkbox"${cState.wF && cState.wF.wholeOnly ? ' checked' : ''}> only the rows Choose early, read late is showing</label>
...
function cSplitPanel() {
  ...
  return `<div class="panel" id="cSplitWrap">
    ...
    : `<input type="checkbox" class="cspick" data-key="${esc(`${p.coin}|${p.geometry}|${p.wholeLookback}|${p.wholeBand}`)}"${cWalkPick.has(`${p.coin}|${p.geometry}|${p.wholeLookback}|${p.wholeBand}`) ? ' checked' : ''} title="tick to promote this pair's WHOLE-HISTORY row &mdash; the one at whole look-back and whole band, which is the setting to tune with. It is the same tick as the one on that row up on Walk it forward, and the same list: promote them with Promote the ticked rows up there.">`}</td>
      ...
      <td>${p.lateWindowsPaid} of ${p.lateWindows}</td>
...
function cWalkBind() {
  ...
  // placeholder cannot be taken and trimmed -- this puts it in the box.
  if ($('#wBacksAll')) {
    $('#wBacksAll').onclick = () => {
      const el = $('#wBacks');
      if (!el) return;
  ...
  // so a row promoted here and a row promoted by hand are the same promotion.
  {
    ...
    if (b && said) {
      b.onclick = async () => {
        ...
        cState.wFrom = 0;
        cShownFor = null;
        cSplitSaid = `${keys.length} row(s) promoted \u2014 the table above is showing those ${keys.length} rows and nothing else. `
          + 'Untick only the rows Choose early, read late is showing to widen it again.';
        cRemember();
...
const C_FIELD_OF = {
  ...
  scrambles: (p) => (p.now ? p.now.scramblesAsGood : null),
  slides: (p) => (p.now ? p.now.slidesAsGood : null),
};
...
function cFieldPairRow(p, shapes) {
  const label = (k) => (shapes.find((x) => x.key === k) || {}).label || k;
  const s = p.state || null; const r = p.range || null; const f = p.now || null;
  ...
  return `<tr>
      ...
      <td>${f ? `${f.scramblesAsGood == null ? '—' : f.scramblesAsGood} / ${f.slidesAsGood == null ? '—' : f.slidesAsGood} of ${f.copies}` : (s ? '<span class="muted" title="this pair was built before 3.216.0, when the null sets were taken once on the fill day and never again; press Build the field to have them read over the current window">build again</span>' : '—')}</td>
...
// A BUY BELONGS TO ITS FIELD (owner, 2026-09-21: "if a buy has been created
// for a field, it's connected to the field, and it displays when you open the
// field. And if the field is not open, it does not display"). The buys on
// screen are the open field's own, newest press first; none while no field
// is on the screen.
function cBuysOfOpenField() {
  const saved = cFieldSt && cFieldSt.saved ? cFieldSt.saved : null;
  if (!saved) return [];
  return (cBuysNow || []).filter((b) => b && b.field && b.field.id === saved.id);
}
function cBuyPicked() {
  const list = cBuysOfOpenField();
  return list.length ? list[0].id : null;   // the open field's own buy, its newest press
}
...
function cFieldBuyBlock(st, off) {
  ...
  if (!saved) return '';                      // no field on the screen: nothing of the buy is shown
  const buys = cBuysOfOpenField();
...
function cFieldPanel() {
  ...
  return `<div class="panel">
    ...
    <hr style="border:0;border-top:1px solid var(--line);margin:1.8rem 0 1.2rem">
    <p class="note"><b>Every pair of ${esc(String((st && st.saved && st.saved.id) || ''))}</b>${st && st.saved && st.saved.name ? ` &middot; ${esc(String(st.saved.name))}` : ''}: one row per coin and chunk shape, read on its last day.</p>
    <div class="cwbox cwtall"><table class="cgap cpassers"><thead><tr>
      ...
      <th title="on the last day, over the current window: how many scrambled copies (outcomes dealt into a random order, each rolled from the start to the last day) did at least as well as the real field, and how many slid copies did, of the copies built. Low is the result. Read again on every build.">scrambles / slides as good now${cFieldSortBtn('scrambles', 'asc')}</th>
      <th></th>
    </tr></thead>
...
function cFieldBind() {
  ...
  if (buy) buy.onclick = async () => {
    ...
    try {
      await post(`api/coins/fields/${encodeURIComponent(id)}/buy`, {});
      await cBuysLoad();
...
// a bar of three thousand decisions is a few hundred fills. The hover names the
// decision under the pointer: its day, its window move and how it reads.
function cPaintBars(recs) {
  const byCoin = new Map(recs.map((r) => [r.coin, r]));
  for (const cv of document.querySelectorAll('canvas.cbar')) {
    const r = byCoin.get(cv.dataset.coin);
    const s = r && r.shapes && r.shapes[cv.dataset.shape];
    if (!s || !s.reading) continue;
    const W = Math.max(50, cv.clientWidth || 0);
    const H = 18;
    cv.width = W; cv.height = H;
    const g = cv.getContext('2d');
    g.fillStyle = C_TRACK; g.fillRect(0, 0, W, H);
    const n = s.reading.length;
    const unit = W / n;
    let i = 0;
    while (i < n) {
      let j = i;
      while (j + 1 < n && s.reading[j + 1] === s.reading[i]) j++;
      const x0 = Math.floor(i * unit);
      const x1 = Math.max(x0 + 1, Math.ceil((j + 1) * unit));
      g.fillStyle = C_COLOUR[s.reading[i]] || C_TRACK;
...
async function drawCoins() {
  ...
  const now = await apiOr('api/data-state', null);
  const cachedNow = new Map(((now && now.symbols) || []).map((s) => [String(s.symbol).toUpperCase(), s]));
  const shapes = (d && d.shapes) || [];
  ...
  const behind = (r) => {
    ...
    if (!c || !r.provenance || r.provenance.cachedMonths == null) return 0;
  ...
  ${!recs.length ? `<div class="panel"><p class="note">no coin has been read${unreadable.length ? ' that this release can draw' : ''} — press <b>Read these coins</b> above</p></div>` : recs.map((r) => {
    ...
    return `<div class="panel ccoin">
      <h3 style="margin-top:0">${esc(r.coin)} <span class="muted" style="font-weight:normal;font-size:.74rem">read ${esc(cWhen(p.capturedAt))} UTC · release ${esc(String(p.release || '—'))} · ${p.candles || 0} candles${grew ? ` · <span class="warn">${grew} more month(s) cached since</span>` : ''}</span></h3>
      ${r.read ? shapes.map((sh) => cShapeBlock(r.coin, sh, r.shapes && r.shapes[sh.key], layouts)).join('') : `<p class="note warn">${esc(r.why || 'could not be read')}</p>`}
    </div>`;
  }).join('')}
  </div>
  ...
  const cc = $('#cCoins');
  if (cc) {
    cc.oninput = () => { cState.coins = cc.value; cRemember(); };
    cc.onchange = () => { cState.coins = cc.value; cRemember(); };
  }
  ...
  if (cb) {
    cb.onchange = async () => {
      try { await post('api/coins/band', { band: cb.value }); } catch (err) {
        $('#cOut').innerHTML = '<span class="warn">' + esc(err.message) + '</span>';
        return;
      }
      draw();
  ...
  $('#cRun').onclick = async () => {
    ...
    try { await post('api/coins/run', { coins: cState.coins }); } catch (err) {
      $('#cOut').innerHTML = '<span class="warn">' + esc(err.message) + '</span>';
      $('#cRun').disabled = false;
      return;
    }
  ...
  if ($('#cStop')) {
    $('#cStop').onclick = async () => {
      $('#cStop').disabled = true;
      let ans = null;
      try { ans = await post('api/coins/stop', {}); } catch (err) { ans = { stopping: false, why: err.message }; }
      if (ans && !ans.stopping) $('#cOut').innerHTML = '<span class="warn">' + esc(ans.why || 'nothing was stopped') + '</span>';
      else $('#cOut').innerHTML = 'stopping after the coin being read now…';
    };
  }
  if (coinsPoll) { clearTimeout(coinsPoll); coinsPoll = null; }
  if (running) coinsPoll = setTimeout(cPollStatus, 2000);
}
