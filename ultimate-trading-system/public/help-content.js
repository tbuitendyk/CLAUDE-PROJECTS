...
window.HELP = {
  ...
  coins: {
    ...
    controls: {
      ...
      cAuto: 'ticked, every coin and shape is drawn at its own sweet spot: the band inside its plateau that keeps the most edge per decision (edge per called trade times the share of decisions still called), smoothed three bands wide, the lower band on a tie. Where the signal line says no band beats chance for three steps together, the typed band applies, and the bar\'s heading says which band it is drawn at and why. Unticked, the typed band applies everywhere. This only changes which band the bars are drawn at: nothing is read again either way, nothing outside that section reads it, and it lives beside the band in the same home.',
      cGridFrom: 'the lowest band the plateau is searched from. A plateau is the run of bands that beat chance together, and sweet spot band is picked inside it, so these three boxes set the whole range that column can take. At 0 nothing sits out, and a plateau that starts there is one whose edge does not need a sit-out band at all.',
      cGridTo: 'the highest band the plateau is searched to. A band above this one can never be the sweet spot band however well it would have read, so it wants to reach at least as high as bands to try on Walk it forward — otherwise the two halves of this screen are searching different ranges.',
      cGridStep: 'how far apart the bands are. Every band in the range is scored on every coin and every chunk shape each time the coins are read, so halving the step doubles what a reading costs; the line beside the boxes says how many bands the three of them come to, and the most it will accept. Like the look-backs, the search happens when the coins are read, so a change only reaches the readings the next time you press Read these coins.',
      cBacks: 'the look-backs a reading stores, in hours, comma separated. The chunk shape decides the TRADE — when it opens and how long it is held — and up to now it also decided what was LOOKED AT, which there was never a reason for: a Daily 2-day trade can be judged on the last two days, or the last week, or the last fortnight. These are measured from candles when the coins are read, so changing them only reaches the records the next time you press Read these coins, and the line beside the buttons says so. Every look-back ends at the same moment the decision is taken, so none of them can see past it.',
      wBacks: 'which look-backs to walk, in hours, comma separated. Blank walks only each chunk shape\'s own span, which is what every reading did before look-backs existed. A look-back the records do not carry is left out rather than guessed at — the greyed text in the box lists what they do carry. Every one you add multiplies the table, so the scrambled copies matter more, not less.',
      ...
      wBands: 'how big a move has to be before it counts, as a percentage of the coin\'s usual move, comma separated. Every one of them is walked and every one is reported — the point of the walk is that nothing is searched for, so the best is never kept quietly on its own.',
      ...
      wBacksAll: 'puts every look-back the records actually carry into look-backs to try, hours, so you can see what is there and then trim it to what you want walked. The placeholder under that box has always shown the same list, but a placeholder cannot be taken and edited. A look-back the records do NOT carry cannot be walked at all — the walk leaves it out rather than guessing it — so to add one, put it in look-backs to store, hours in the reading above and press Read these coins first. That is the only press that measures a look-back from the candles.',
