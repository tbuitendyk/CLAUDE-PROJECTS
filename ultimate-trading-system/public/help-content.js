...
window.HELP = {
  ...
  coins: {
    ...
    controls: {
      ...
      fBuy: 'reads every coin and shape pair of the field open on this screen and writes down the seven best candidates for their newest decision, with the price each trade opened at. A pair is a candidate when its field was full on its last day, said a direction, at least one point spoke and certainty could be read; candidates rank by certainty times agreement, evidence breaking ties. The record is fixed as pressed; only the price and the performance move as candles arrive. Nothing is traded.',
      fBuyPick: 'every buy on this box, newest press first, with the field it read, when it was pressed and how many candidates it found.',
      fBuyOpen: 'reads the chosen buy again with each trade\'s price as it stands: the newest candle on file while the trade runs, the exit price once the exit candle is on file.',
      fBuyDel: 'deletes the chosen buy. It asks once; the record of that press cannot be made again once the data has moved on.',
