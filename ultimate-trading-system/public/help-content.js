    ...
    return {
      ...
      how: [
        ...
        ['The tunings applied on Tune',
          `A stop forced onto a survivor on Tune, and the conviction sizing applied to it there, are frozen into the ${kind} a press writes, and the press prices that survivor again under them: its captured trades on the ${w} window, plain and with the stop and the sizing applied, by the two scans' own arithmetic, in the same dollars as the rest of the table, the record's own; the scans on Tune print one clip of $10, so their figures read a tenth of these. Both numbers sit side by side in the survivors table, with how many trades the stop closed early. The verdict reads the plain money, because the scrambled copies are priced plain and a tuned survivor against plain copies would be two different things compared. A rule with no capture on Tune, or a reserve read on a capture that holds no reserve entries, says so on the set and prices nothing; capture the trades on Tune and press again. Apply a tuning or take it off on Tune and read again: each press freezes what was on record at that moment, and a greenlight carries it. The capture's own plain re-pricing is held against the reading's money to the cent, and a gap is said on the set.`],
