    ...
    return {
      ...
      how: [
        ...
        ['The tunings applied on Tune',
          `A stop forced onto a survivor on Tune, and the conviction sizing applied to it there, are frozen into the ${kind} a press writes, and the press applies them: that survivor's captured trades on the ${w} window are priced again under the stop and the sizing, by the two scans' own arithmetic, and every reading on the set then reads it at that money. Its money without them stays in the survivors table beside it, with how many trades the stop closed early. One currency everywhere: the record's own dollars, $100 a trade, on Tune's scans, on these readings and on Greenlight's picture alike. The scrambled copies are priced plain, because a stop or a sizing on a survivor is not on its copies, and the set says so. A rule with no capture on Tune, or a reserve read on a capture that holds no reserve entries, says so on the set and reads every survivor plain; capture the trades on Tune and press again. Apply a tuning or take it off on Tune and read again: each press freezes what was on record at that moment, and a greenlight carries it. The capture's own plain re-pricing is held against the record's money to the cent, and a gap is said on the set.`],
