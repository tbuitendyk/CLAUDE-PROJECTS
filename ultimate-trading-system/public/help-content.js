    ...
    return {
      ...
      how: [
        ...
        ['The tunings applied on Tune',
          `A stop forced onto a survivor on Tune, and the conviction sizing applied to it there, are frozen into the ${kind} a press writes, and the press reads each survivor at its money under both. The stop changes WHICH trades happen and at what price; the sizing changes HOW MUCH each one bets, each trade at its own size times the multiplier of its row, and a multiplier of 0 leaves a trade out. That money, over the trades the sizing takes, is what every figure on the set reads for the survivor: the average, the four comparisons, and its own copies. The null sets - the scrambled copies - are read at sizing 1, one clip a trade. The survivors table prints the money without the tunings beside it, and the clips a trade the sizing deploys. One currency for all of it: the record's own dollars, $100 a trade. A rule with no capture on Tune, or a reserve read on a capture that holds no reserve entries, says so on the set and reads every survivor plain; capture the trades on Tune and press again. Apply a tuning or take it off on Tune and read again: each press freezes what was on record at that moment. The capture's own plain re-pricing is held against the record's money to the cent, and a gap is said on the set.`],
...
window.HELP = {
  ...
  sweep: {
    ...
    controls: {
      ...
      swPlainUnits: 'run EXACTLY the same coins and chunk shapes the choice above gives, and leave the extra members out. It is the control arm, and it is the only way to get one. A promoted row carries a look-back and a band of its own, and that becomes one more member on its unit; to know whether that member is worth anything you need two runs that differ in NOTHING ELSE. Choosing ignore what is on Coins instead builds the units from the boxes on this screen, which is a different list of coins and chunk shapes altogether \u2014 so the two runs would differ in what was traded as well as in how many members voted, and neither difference could be read off the other. The set records which arm it was, and the lines above hold the two up against each other, so a set run with the extra members and one run without can never be mistaken for the same run. Greyed unless the units come from a walk set, because only a walk set adds extra members to leave out \u2014 a row under coins and shapes that pass is a coin and a chunk shape and nothing more.',
      swExtraShare: {
        what: 'How the history is cut for a member added from a walk set: it trains on the first share of everything the run reads and is read on the rest. 60/40 trains on the first sixty percent and reads the member on the last forty; 50/50 halves it. Greyed when the run has no extra members.',
        more: 'The window layout is not touched by this. It still cuts the history for every member the unit was always going to have, exactly as before, and the sealed exam stays sealed: the share is taken of what the layout leaves unsealed \u2014 train, test and held-back together, in order. A member added from a walk set is the exception, because the walk already SET its look-back and band on the first half of this same history and CONFIRMED them on the second; there is nothing left for a small test slice to choose, and on a fifteen percent slice its gate opened on a handful of decisions, which reads nothing. So it trains on its share and is read on the whole of the rest. The committee\u2019s own test and held-back windows lie inside that rest, so its votes there are still votes on decisions it never trained on, and it votes as every other member does. The tuning-slice $ still read every member on the same slice. The set records the split, stage 2 inherits it so both halves of a committee are cut the same way, and a set made before the split existed cannot be carried to stage 2 \u2014 it says so, and stage 1 is run again.',
      },
      ...
      swPlateauShare: {
        what: 'Of a plateau\u2019s trained members, the share that must call the same side before the plateau\u2019s one vote is cast. At 50% five of nine must agree; at 100% all of them. Greyed when no unit being priced carries a plateau.',
        more: 'A plateau is the nine around a promoted row \u2014 the row itself and its neighbours one step each way in look-back and in band on the walk set\u2019s own grid. Stages 1 and 2 train all nine, and here they vote ONCE per kind, LOGREG and BOOST, however many of them there are: nine readings of one piece of evidence are not nine pieces of evidence. The plateau\u2019s lean is its members\u2019 votes added and shared out, whatever this box says, so a way of weighing that reads how hard a member leans sees how firmly the region agrees. Its call is cast only when this share of its trained members call the same side, so a way of weighing that counts sees one vote, and only when enough of the nine agree. A member that could not be trained is left out of both, because a member with no opinion is not a neutral one. The share is written into every setting\u2019s name, is stored on every record, and travels on a greenlight so the live path folds the plateau the way it was priced. On a run with no plateaus it is stored as nothing.',
      },
      swPermPlateauShare: {
        what: 'Price every plateau share as its own setting in the block.',
        more: 'On a unit that carries no plateau every value of it places the same trades, so those fold into one setting there, the way confirm folds on a unit without a lean.',
      },
      swConfirm: {
        what: 'What the coin\u2019s own lean changes about the trades, on a unit whose coin and chunk shape came from Coins \u2014 under coins and shapes that pass or from a walk set. off changes nothing: every trade at size 1. confirmed only drops every call the lean disagrees with. strictly confirmed drops those and every call the lean has nothing to say about, so only trades the lean actively agrees with are placed. sized trades a call the lean agrees with at confirmed \u00d7 the size and one it disagrees with at unconfirmed \u00d7 the size.',
        more: 'The members decide every trade first, through the quorum; the lean only gets a say afterwards, trade by trade, and can never start one. For a passer the lean is read the way Coins reads it: at the unit\u2019s own sweet spot band on the shape\u2019s own span, each window is rising, falling or inside the band, and the coin\u2019s history says which way a trade should go after rising and after falling. For a unit promoted from a walk set the lean is its PLATEAU\u2019S: every row of the plateau reads its own window at its own look-back and band against its own yardstick and gives a side, or nothing where its window sat out, and the plateau leans a side when at least the plateau share of its rows say that side \u2014 the way its vote is folded, so one selected row cannot bias it. Those leans ride on the unit\u2019s own record from the stage 1 launch; a row walked before rows kept a lean is left out. A call made when the lean has nothing to say is a no-lean call: size 1 under off, confirmed only and sized, dropped under strictly confirmed. A trade at twice the size is exactly twice the money, fees included, so the six numbers behind each row \u2014 money and count of the confirmed, the unconfirmed and the no-lean trades \u2014 say whether the lean adds anything: Boards prints a verdict word beside every setting and every coin. On a unit that carries no lean the values place the same trades and are priced once. Greyed while no unit being priced carries one.',
      },
      ...
      swField: {
        ...
        more: 'On each decision the field is read on that decision\u2019s own day, and the trade is blocked when the field\u2019s sign is against the members\u2019 call, blocked when a minimum is not reached unless sign only is ticked \u2014 a bar on the field\u2019s agreement, a bar on its certainty, or both bars combined as must pass says \u2014, and otherwise sized by the rung the read falls in. A unit whose coin and shape has no pair in the field prices plain, and every value of the gate is one setting there. The field takes confirm\u2019s place: a run names one or the other, never both, and the launch refuses the pair in words. What the run read is frozen beside the record set at the launch, so deleting or rebuilding the field on Coins afterwards changes nothing about what was priced.',
      },
      swFieldAgreeMin: { what: 'The least the field\u2019s agreement must reach on the decision\u2019s day for a trade to be placed, 0 to 100 \u2014 how much of the field\u2019s pull pointed one way. One number, or a comma-separated list with permute ticked to price each as its own setting. 0 blocks nothing; blank is no bar on agreement. The sign block still applies.' },
      swPermFieldAgreeMin: { what: 'Price every agreement minimum in the box, each as its own setting.' },
      swFieldCertMin: { what: 'The least the field\u2019s certainty must reach on the decision\u2019s day for a trade to be placed, 0 to 100 \u2014 how the field\u2019s size ranked against its slid copies that day, so only a field built with copies has it; the launch refuses a bar here on a field built without, and the line under the dials says so as it is typed. One number, or a comma-separated list with permute ticked. 0 blocks nothing; blank is no bar on certainty.' },
      swPermFieldCertMin: { what: 'Price every certainty minimum in the box, each as its own setting.' },
      swFieldRule: {
        what: 'How the two minimums combine when both boxes hold a number. both: a trade needs both bars reached, and missing either blocks it. either: reaching one bar is enough, and only missing both blocks it. With one box blank there is one bar and this choice changes nothing.',
        more: 'This is what the two bars are for together: one setting asking for agreement at least 40 and certainty at least 60 is the only way to learn whether certainty adds anything beyond agreement, and Boards shows its answer beside the settings that asked for one bar alone. With one bar the rule is not permuted, since it would price the same trades twice under two names.',
      },
      swPermFieldRule: { what: 'Price both ways of combining, each as its own setting.' },
      swFieldSignOnly: { what: 'Ignore both minimums: block only when the field\u2019s sign is against the members\u2019 call, and size every other trade by its rung. The owner\u2019s override, so a field that rarely reaches a bar can still steer size.' },
      swPermFieldSignOnly: { what: 'Price both with and without sign only, each as its own setting.' },
      swFieldRead: { what: 'Which of the field\u2019s two numbers the size rungs read to size a placed trade. agreement: how much of the field\u2019s pull pointed one way on that day, 0 to 100. certainty: how the field\u2019s size ranked against its slid copies that day, 0 to 100 \u2014 only on a field built with copies; the launch refuses it otherwise. The minimums are bars on each number by name and do not follow this choice.' },
      swPermFieldRead: { what: 'Price both reads, each as its own setting, so sizing by agreement and sizing by certainty sit side by side on Boards.' },
      swFieldRungs: {
  ...
  boards: {
    ...
    intro: 'Where the record sets are read: one sub tab per stage, Stage 1, Stage 2 and Stage 3, and the whole provenance '
      ...
      + 'Stage 3 or one of its tables is picked, three more tabs stand beside Stage 3, a little apart: Table 3.A, Table '
      + '3.B and Table 3.C. Stage 3 shows the record set, down to Check this set; each table tab shows its table alone, '
      + 'starting with the table\'s own title, for the stage 3 record set picked on Stage 3. The show the held-back '
      + 'window tick sits under the title of Table 3.A and of Table 3.B, the tables it reaches. The tabs picked are '
      + 'remembered. No table mixes two stages.',
    how: [
      ['One table per stage, and the chain always visible',
        ...
        + 'The head count in the members column of the stage 1 and stage 2 tables opens. Press it and that one '
        + 'unit\'s committee appears under the table, one row per member: which of the two ways of working out a '
        + 'forecast it uses, which slice of the numbers it reads, what it is marked at, and how each one did on '
        + 'its own \u2014 its own forecast score, how many of its own null set it beat, and its lead over them. Two '
        + 'more columns say how often it spoke at all and how often it was right when it did, which is what tells '
        + 'you apart a member that has learned something from a member that has learned to say nothing. A member '
        + 'added from a walk set is marked with a line down its left edge and carries the look-back, the band and '
        + 'the walk set it came from. One unit can be open on each table at a time.\n\n'
      ...
      ['Table 3.C: Every unit, and the filter the Funnel reads',
        ...
        + 'field\u2019s gate and per trade, how the middle and the best setting did, how many settings are in the money in '
        + 'each of the three parts of the test window and in all three, how many are losing in all three parts, the four '
        + 'comparisons Worth walking? reads, what the top of the order made in the third part, how the whole board does '
        + 'against the kept scrambled copies, how many settings beat the best of the four \u2014 being long every period, '
        + 'being short every period, buying the coin and going away, and shorting it and going away \u2014 at their own '
        + 'hold length, how often it trades, how much the field blocked, the worst losing streak and the trades won '
  ...
  tune: {
    ...
    controls: {
      ...
      tnPick: {
        ...
        more: 'By depth is the survivor most surrounded by neighbouring settings that survived too, among the captured survivors, chosen without looking at money: the same pick Greenlight makes, so a scan can be aimed without shopping the survivors. All survivors pools every captured survivor\'s trades into one list, each trade held for its own survivor\'s length, so the scan reads the whole table at once. Naming one records it as your pick.',
      },
  ...
  coins: {
    ...
    controls: {
      ...
      // THE DECISION FIELD (FIELD-DESIGN.md), the section under Walk it forward
      fWindow: 'how many days the decision field remembers, for every coin. A number is held to the most this box allows — the train stretch of the shortest history under 61/13/13/13, printed beside the box — so the window is always full by the time a test day is traded, whichever window layout a run picks. Days older than this leave the points; days inside it are weighed by the half-life beside it.',
      fEachOwn: 'ticked, each coin\'s window is its own train stretch under 61/13/13/13 rather than the one number typed beside it, so a coin with eight years of history remembers more than one with six; the number box is greyed. Unticked, every coin remembers the same number of days, which is what makes their fields comparable.',
      fHalf: 'the age, in days, at which a day\'s weight in the points has halved. The day being read is full weight; a day this old counts half, twice this old a quarter, and so on down to the weight floor. The coin\'s usual move, which every sit-out band is a share of, is weighed the same way over the same window.',
      fFloor: 'the least weight any day in the window keeps however old it is. At 0.1 the oldest days count a tenth of today; at 1 the half-life does nothing and every day in the window counts the same.',
      fCap: 'the most evidence one point may bring to the field, in full-weight days. A point with more evidence than this counts as though it had exactly this much, so no single point can carry the field on its own. 0 turns the cap off.',
      fLeast: 'the least evidence a point needs, in full-weight days, before it speaks on a day its reading would let it. A point with less says nothing.',
      fCopies: 'how many slid copies of each coin are rolled beside the real one. A slid copy moves every outcome along by the same amount and wraps the tail round to the front, so each outcome keeps its neighbours in time and only which reading it sits under is cut. Certainty on a day is how many of the copies the real field\'s size beat, as a share of them. The scrambles count — the outcomes dealt into a random order instead — is taken once, on the day the window first fills, and shown beside the slides count in the table; nothing but that column reads it. Zero skips both.',
      fBandFrom: 'the lowest sit-out band the grid starts from, as a percentage of each look-back\'s usual move on the day being read. Filling these three boxes in and pressing Apply writes a list into the sit-out bands box; that list is what is built.',
      fBandTo: 'the highest sit-out band the grid reaches.',
      fBandStep: 'how far apart the bands are. Every band in the list is one column of the grid, so halving the step doubles the points.',
      fBandApply: 'puts the three boxes beside it — lowest, highest, step — into the sit-out bands box as a list. Nothing is built; the list is yours to edit afterwards.',
      fBands: 'the sit-out bands of the grid, comma separated, and the only thing that decides them. Apply fills it from the three boxes above; after that it is yours to edit — leave gaps, add one on its own, take one out. Each is a percentage of the look-back\'s usual move: at 100 a decision reads rising or falling only when it moved more than the coin usually does over that look-back, and sits out otherwise. A point with a wide band speaks rarely and a point with a narrow one often; neither is right or wrong for it, the evidence says which.',
      fBackFrom: 'the shortest look-back the grid starts from, in days. Filling these three boxes in and pressing Apply writes a list into the look-backs box; that list is what is built.',
      fBackTo: 'the longest look-back the grid reaches, in days.',
      fBackStep: 'how far apart the look-backs are, in days. Every look-back in the list is one row of the grid.',
      fBackApply: 'puts the three boxes beside it — shortest, longest, step — into the look-backs box as a list. Nothing is built; the list is yours to edit afterwards.',
      fBacks: 'the look-backs of the grid, in days, comma separated, and the only box for them. A look-back decides what is LOOKED AT — the move into the decision over that many days — and the chunk shape decides the trade. Every look-back ends at the decision instant, so none can see past it.',
      fCoins: 'which coins to build the field for, comma separated. Blank builds every coin that has been read on this screen.',
      fGeom: 'the chunk shape the field is built for. The shape decides the trade the outcome is measured over — when it opens and how long it is held — and the days the decisions fall on. Greyed while permute is ticked, because then every shape is built.',
      fPermGeom: 'ticked, every chunk shape is built rather than the one chosen beside it: one field per hold length, as the walk does, each standing for the shapes that hold for the same time. It is the same tick stage 1 on Sweep uses to run every shape.',
      fName: 'the name the field this build writes will carry. Blank takes the greyed name beside it. Every finished build writes a field to disk under it; a build you Stop does not, but what it saved is kept, listed as unfinished, and can be carried on.',
      fSet: 'every finished build on this box, newest first, with how many coin and shape pairs it holds, what it takes on disk, when it finished, which release built it and the dials it was built under. Opening one puts its table on this screen exactly as a fresh build would be. A stage 3 run on Sweep names one of these to gate with.',
      fSetOpen: 'puts the chosen field\'s table on this screen. Whatever build was in hand is replaced; nothing on disk is touched.',
      fSetClose: 'puts the field on this screen away — opened or just built — so its table leaves the screen. Nothing on disk is touched; Open this field brings it back. A build that is going is stopped first, never put away.',
      fSetName: 'renames the chosen field. The id stays; the name is what the box shows beside it.',
      fBuy: 'reads every coin and shape pair of the field open on this screen and writes down its seven best candidates for their newest decision, with the price each trade opened at. A pair is a candidate when its field was full on its last day, said a direction, at least one point spoke and certainty could be read; candidates rank by certainty times agreement, evidence breaking ties. The seven are fixed as pressed and shown under the button whenever this field is open; only the price and the performance move as candles arrive: the newest candle on file while a trade runs, the exit price once its exit candle is on file. One field, one test: the button is greyed once the field has its seven, because a field\'s last day is fixed when it is built and a second press would say the same. A new day needs a refresh on Data and Build the field again, and the new field has its own press. Deleting a field takes its seven with it. Nothing is traded. The line under the seven is their average performance, %, worked out from the rows as they stand on every draw \u2014 a running trade at its newest price, a closed one at its closing price \u2014 so it moves as trades close and is never stored.',
      fBuyUpdate: 'fetches the newest closed hour for every trade of the seven that is still running, the way Refresh to latest fetches it, closes any trade whose closing has passed at its closing price, and reads the table again. Live only while a trade of the seven is still running; greyed once every trade is closed. Refused in words while a data job holds the cache.',
      fSetDel: 'deletes the chosen field, in two steps: the first press says what would go, and only the field\'s own id typed back does it. Hours of compute cannot be got back from a mis-click.',
      fRun: 'builds the field for every coin and shape named above, one task per pair across every worker the box is set to use, and writes each pair to disk the moment it lands. The line beside the button counts the pairs off and says how busy the box is; leaving the tab and coming back finds it still going. When every pair has landed the build becomes a field, named in the box above, and its table appears below: each pair\'s state at its last day, the range of its agreement and certainty over the window, the two copy counts at fill, and a button that opens its grid.',
      fStop: 'stops the build where it is. What has already landed is kept as an unfinished build, listed under the button, and can be carried on later; it is not a field until every pair has landed.',
      ...
      wPromote: 'puts every row ticked in the leftmost column of Walk it forward into the list at the top of Coins, under the walk set it came from. A tick on Choose early, read late below ticks a row here, so the two tables share one list and this one press carries all of it, in one ask however many there are. It is a REFERENCE, not a copy: the row stays on its walk set and is read back off it, so deleting that set takes these rows with it and nothing at the top can go stale against a re-walk. A walk has to be saved before a row of it can be promoted, because there has to be a set to reference. Promoted rows arrive ticked; untick one at the top to leave it out of a sweep without giving up the promotion.',
      sPromote: 'promotes the WHOLE-HISTORY pick of every row Choose early, read late is showing \u2014 the look-back and band in the four columns on the right, not the look-back and band the early windows chose. The early pick is the test: it spends half the history to answer whether the choosing is worth anything at all. The whole-history row is what a unit that has passed there should actually be set to, and it is the row the jump button marks. They go to the list at the top of Coins through the same door a row ticked by hand goes through, so the two are one promotion and nothing already promoted is promoted twice. The line beside it says how many of the rows carry a look-back in hours \u2014 those add one more member to their unit on a sweep \u2014 and how many read the chunk shape\u2019s own span, which add none and whose unit runs as a plain one; it asks before it promotes and says both numbers again there. A pair the whole history could not choose a row for is left out. It is not drawn at all on a reading of the walk in hand, because promotion is written on a walk set and a walk that has not been saved has none.',
      ...
      // A NAMED SET OF FILTER BOXES, KEPT ON THE BOX (3.180.0)
      wScreen: 'every screen kept on this box. A screen is the filter boxes on BOTH tables — Walk it forward\u2019s and Choose early, read late\u2019s — saved together under a name. It is kept on the box rather than in this browser because it is a decision, not a place you are standing: a rule read one way here and another way on another machine is two rules.',
      wScreenOpen: 'puts the chosen screen on: every filter box on both tables is replaced by what that screen holds, and both tables redraw. It changes no reading and no record — only which rows are shown. The line beside these buttons says how many boxes the screen holds before you press it.',
      wScreenSave: 'keeps the filter boxes on BOTH tables, exactly as they stand, under a name you type. Saving under a name that already exists replaces it. Why it is worth doing: the rule you select on is the thing that should be fixed BEFORE the numbers are looked at, and typing eleven boxes again on the next walk is how a rule quietly becomes whatever this morning\u2019s table happened to look like.',
      wScreenName: 'gives the chosen screen a different name. The boxes it holds are untouched, and a name another screen already has is refused.',
      wScreenDel: 'removes the chosen screen from this box. The filter boxes on both tables are left exactly as they are, so nothing on screen changes except that the name is gone. One confirm is enough here — a screen is a set of boxes, not hours of compute the way a walk set is.',
      ...
      sf_maxWholeGood: 'hide rows whose WHOLE-history row more than this many scrambled copies matched, out of however many the walk dealt. That row is the one a tick on this table promotes, so this is the copy count that decides what you carry forward — the pick the early windows made has no copy count of its own here. Low is the result: with dozens of rows on a table, merely positive is not.',
      sf_maxWholeSlid: 'the same box against SLIDING copies. A sliding copy moves every outcome along by the same amount and wraps the tail round to the front, so each outcome keeps the outcomes it actually happened next to and the only thing cut is which reading it sat under. The scrambled copies deal them into a new order instead, which also destroys the run of the outcomes themselves. On a coin that simply drifts, that makes the dealt copies far harder to beat than they should be while the slid ones come out fair — so where the two disagree, trust this one.',
      ...
      sSkipOwn: 'leave out every row whose look-back is the chunk shape\u2019s own span, BEFORE anything is chosen. A row at its own span reads the same window the members of a committee already read, so promoting it adds no member to its unit \u2014 the unit still runs, it just runs as a plain one, and what the walk found does not travel with it. On a set where most of the best rows are at their own span, a reading over every row therefore points at rows that cannot carry anything. TICKED, THE EXCLUSION IS PART OF THE CHOOSING, not a column of second-best alternates added afterwards. That is the whole point of putting it here: every figure the reading then reports \u2014 the pick, picking blind, the lead, the percentile, the four whole-history columns and the pass count \u2014 is about the same procedure you would actually use. An alternate column would hand you a row to promote whose selection had never been tested, which is the kind of hindsight this reading exists to catch. AND THE NUMBERS MAY COME OUT WORSE. On a pair whose best row was its own span, the restricted choosing takes second best, and on a pair whose only rows with enough trades were at its own span there is nothing left and the pair drops out of the reading altogether \u2014 so the pair count falls too. If the count and the percentile fall, that is the honest answer: the edge was in rows that add no member.',
      wf_wholeOnly: 'keep ONE ROW for each coin and chunk shape Choose early, read late is showing \u2014 the row at its whole look-back and whole band. That is the setting to tune with, and it is the row Promote every row shown sends, so 24 pairs on that reading means 24 rows on this table. It reads the reading through the same key the press builds, so the table cannot show a different set from the one that went. IT IS NOT EVERY ROW OF THOSE PAIRS. A pair has a row for every look-back and band the walk tried, which runs to hundreds, and keeping all of them for every shown pair puts tens of thousands of rows on screen \u2014 which is the walk again, not a review of what was chosen. To see everything one pair did, use its own jump button on the reading, which sets the coins and chunk shapes boxes to that pair alone. Pressing Promote every row shown turns this on and brings you here. Until you press Choose early, read late there is no reading to filter on and the tick hides nothing.',
