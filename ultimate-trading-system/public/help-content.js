  ...
  funnel: {
    ...
    controls: {
      ...
      fSetRebuild: {
        ...
        more: 'Six numbers are not stored by a sweep - the worst losing streak, the biggest single loss, the best single trade, how many trades won, how many were stopped out, and the gross per trade. They are worked out on the press at step 6 of a walk and kept in one file beside the STAGE 3 record set. Until this release a later walk over the same stage 3 records wrote that file fresh, so an earlier set lost them: its columns went empty and, if its rule put a limit on the worst losing streak or on trades, its rule stopped keeping anything at all. The file adds rather than replaces now, and every set cut from now on keeps its own copy - but a set cut before that has nothing to copy, and this is the one thing that can put it right. It takes minutes, it waits for any sweep that is running, and the answer is kept on this set for good.',
      },
      fRegionAtLeast: {
