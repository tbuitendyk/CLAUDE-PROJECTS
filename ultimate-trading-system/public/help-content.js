...
window.HELP = {
  ...
  coins: {
    ...
    controls: {
      ...
      fBuy: 'reads every coin and shape pair of the field open on this screen and writes down its seven best candidates for their newest decision, with the price each trade opened at. A pair is a candidate when its field was full on its last day, said a direction, at least one point spoke and certainty could be read; candidates rank by certainty times agreement, evidence breaking ties. The seven are fixed as pressed and shown under the button whenever this field is open; only the price and the performance move as candles arrive: the newest candle on file while a trade runs, the exit price once its exit candle is on file. A press replaces the field\'s earlier seven; deleting the field takes them with it. Nothing is traded.',
