...
window.HELP = {
  ...
  sweep: {
    ...
    controls: {
      ...
      swCap1: {
        what: 'How many ordinary trades the biggest trade may count for. 0 turns the limit off.',
        more: 'One freak trade can otherwise outweigh fifty ordinary ones, and a fit to one trade is not a fit. Measured on a realistic run of two hundred trades the biggest counts about four ordinary ones and this limit never bites; it is insurance against a genuine outlier, such as the week the market fell by half. It only does anything when weighing by money is on.',
      },
      swNull1: {
  ...
  funnel: {
    ...
    controls: {
      ...
      fCutHome: {
        what: 'Drops the unfinished walk under this row and goes back to Home: this section and the one above it, with nothing under them. You are asked first.',
        more: 'It is for one thing - abandoning a rule you were part way through building - and it is live only when there is one to abandon. A Stage 4 record set showing is finished work: its rule was carried through step 7 and written down, so there is nothing to drop and this press is ghosted. The same goes for a walk whose rule has already been cut to a set, and for a walk you have not done anything to yet. When it is live, it always asks, and the question names the coin and shape and the step it is on. What goes is the step, the rule you have built, every step the walk recorded and its marks, and none of it can be brought back. Stage 4 record sets you have already cut are never touched. To keep a walk and read the section above it instead, press "Put away" - that hides the walk and brings it back exactly as you left it.',
      },
      fSetRebuild: {
