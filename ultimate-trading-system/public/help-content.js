...
window.HELP = {
  ...
  coins: {
    ...
    controls: {
      ...
      wSort: 'which way to order the table. Fewest scrambles as good first is the default, because that is the column that says whether a row means anything.',
      wRun: 'walks every coin and shape named above, at every band, one window at a time from the start of its history to the end. At the start of each window the coin\'s usual move and which way it leans are worked out from everything behind that window and then held still while the window is priced, so no figure knew anything it could not have known at the time. Nothing is stored, nothing is traded, and no record is touched — press it again with different boxes as often as you like.',
