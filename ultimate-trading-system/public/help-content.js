...
window.HELP = {
  ...
  coins: {
    ...
    controls: {
      ...
      wBacks: 'the look-backs, in hours, comma separated, and the only box for them. The chunk shape decides the TRADE — when it opens and how long it is held — and a look-back decides what is LOOKED AT, which there is no reason to tie together: a Daily 2-day trade can be judged on the last two days, or the last week, or the last fortnight. Blank walks each chunk shape\'s own span and nothing else. They are measured from the candles when the coins are read, so a look-back added here is not in the records until you press Read these coins — the line beside the box names any that are waiting on that, and a look-back the records do not carry is left out of the walk rather than guessed. Every look-back ends at the same moment the decision is taken, so none of them can see past it.',
      ...
      wBacksAll: 'sets the look-backs box to exactly what the records already carry, so nothing left in it is waiting on a reading before it can be walked. Use it to see what is there and then trim it.',
