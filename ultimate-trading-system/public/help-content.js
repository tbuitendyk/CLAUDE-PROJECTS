...
window.HELP = {
  ...
  coins: {
    ...
    controls: {
      ...
      wf_wholeOnly: 'keep only the one row per pair that Promote every row shown sends \u2014 each pair\u2019s whole-history row, at its whole look-back and whole band. The tick above it keeps EVERY row of the pairs that reading is showing, which is hundreds of rows per pair and the right screen for asking what else a coin did; this keeps only the rows that were promoted, so a promotion can be read back row by row against the rest of the walk. It reads the same reading, through the same key the press builds, so the table cannot show a different set from the one that went. Pressing Promote every row shown turns it on and brings you here. Until you press Choose early, read late there is no reading to filter on and the tick hides nothing.',
