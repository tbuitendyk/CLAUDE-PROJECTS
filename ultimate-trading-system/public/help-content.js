...
window.HELP = {
  ...
  coins: {
    ...
    controls: {
      ...
      wfApply: 'puts every filter box on at once and redraws the table. Greyed out until a box says something different from what the table is already showing, and greyed out again if you type the old value back. Typing into a box does NOT redraw the table on its own — it used to, which replaced the box being typed into and threw the cursor out of it, so only the first character could ever be entered.',
      wfAuto: 'ticked, each filter box goes on the moment you leave it. Unticked, nothing goes on until you press Apply the filters — one wait for the whole set of boxes rather than one wait per box. The same choice the filters on Boards offer.',
