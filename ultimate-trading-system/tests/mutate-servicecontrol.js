...
const GUARDS = [
  ...
  [path.join(ROOT, 'lib', 'stages.js'), '  if (keptAnswers && acc.agreedMiss && !acc.agreedHit) {', '  if (false) {',
    'aRecordPricedUnderAPlateauShareFindsItsAnswerAndAMismatchIsSaid',
    'a set whose records and answers are keyed differently shows a dash with no note, exactly the fault this fixed'],
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "    const src = r.unit ? (rich.unit(r.unit)[r.label] || null) : (allIn ? (rich.blend()[r.label] || null) : null);", "    const src = r.unit ? (rich.unit(r.unit)[r.label] || rich.blend()[r.label] || null) : (rich.blend()[r.label] || null);",
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "    const todo = unit ? units.filter((u) => u.key === unit) : units;", "    const todo = units;",
    'pressingWorkOutTheMissingNumbersPrepsTheWholeRecordSet',
    'the press prices every coin and shape whatever the chooser names'],
  ...
  [path.join(ROOT, 'lib', 'stages.js'), '  const number = had.length + 1;', '  const number = 1;',
  ...
  // ---- the gap (3.125.0) -----------------------------------------------------
  [path.join(ROOT, 'lib', 'coins.js'), "    out.push(Number(Number(c.diffPct).toFixed(4)));", "    out.push(Number(Number(c.c2 == null ? 0 : (c.c2 - c.c1)).toFixed(4)));",
    'aWindowMoveIsFromTheFirstCandleOfTheWindowToTheOpenOfTheTrade', 'the outcome beside each decision is a price difference rather than the chunk\'s own percentage move, so the gap in moves is in dollars on one coin and cents on another'],
  ...
  [path.join(ROOT, 'lib', 'coinsignal.js'), "    const s = signalSummary(cut, geometryKey, layouts, currentBand, { dealt: true });", "    const s = signalSummary(cut, geometryKey, layouts, currentBand);",
    'withTheLinkCutAPlateauIsRareAndTheCheckIsDeterministic', 'the check forgets it is dealing, and the same fault as above'],
  ...
  [path.join(ROOT, 'lib', 'coinsrun.js'), "        rec.shapes[s.key].linkCut = signal.plateauFalseAlarms(rec.shapes[s.key], s.key, layouts(), DEFAULTS.band, LINK_CUT_TRIALS);", "        rec.shapes[s.key].linkCut = null;",
  ...
  [path.join(ROOT, 'lib', 'coinsrun.js'), "      shapesOut[s.key].signal.linkCut = signal.linkCutWorth(shapesOut[s.key].signal.plateau, sr.linkCut);", "      shapesOut[s.key].signal.linkCut = null;",
    'everyCoinTheRunTouchedIsOnDiskAfterwards', 'the check is on the record and never reaches the screen'],
  [path.join(ROOT, 'lib', 'coinsrun.js'), "      shapesOut[s.key].signal = signal.signalSummary(sr, s.key, lays, band);", "      shapesOut[s.key].signal = signal.signalSummary(sr, s.key, lays, DEFAULTS.band);",
  ...
  // Each names the test that READS THE LINE IT BREAKS (RULE EIGHT).
  [path.join(ROOT, 'lib', 'coinsrun.js'), "      if (sig.plateau && sig.sweetSpot && lc && lc.asStrong != null && lc.asStrong <= bar) {", "      if (sig.plateau && sig.sweetSpot && lc && lc.asStrong != null && lc.asStrong < bar) {",
  ...
  [path.join(ROOT, 'lib', 'coinsrun.js'), "  return passersCached().filter((r) => r.ticked).map((r) => ({ coin: r.coin, geometry: r.geometry }));", "  return passersCached().map((r) => ({ coin: r.coin, geometry: r.geometry }));",
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "    if (!coin || !GEOMETRIES[p.geometry]) continue;\n    units.push(...unitsFor([coin], sizes, [p.geometry], compare));", "    if (!coin || !GEOMETRIES[p.geometry]) continue;\n    units.push(...unitsFor([coin], sizes, Object.keys(GEOMETRIES), compare));",
    'aLaunchCanTakeTheCoinsAndShapesTickedOnCoins', 'every pair is built at every shape, five pairs become twenty-five units'],
  [path.join(ROOT, 'lib', 'stages.js'), "  if (passers && !passers.length) throw new Error('nothing is ticked under Candidates for Sweep on Coins — tick some rows there, or untick \"only what is ticked on Coins\"');", "  if (false) throw new Error('nothing is ticked on Coins');",
    'aLaunchCanTakeTheCoinsAndShapesTickedOnCoins', 'a launch with nothing ticked on Coins runs with no units instead of refusing'],
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "    st.agreeBoth, st.agreePersist, hasLean ? (st.confirm || 'off') : 'off'].join('|');", "    st.agreeBoth, st.agreePersist, 'off'].join('|');",
    'theStageThreeCountIsTheLaunchsFoldWithoutTheSettings', 'the fold keeps one value of confirm on a unit that carries a lean while the count says three'],
  [path.join(ROOT, 'lib', 'stages.js'), "    perUnit.push(decisions.length * agrees.length * mine * (hasLean ? confirms.length : 1));", "    perUnit.push(decisions.length * agrees.length * mine);",
    'theStageThreeCountIsTheLaunchsFoldWithoutTheSettings', 'the count says a unit with a lean prices one value of confirm while the fold builds three'],
  [path.join(ROOT, 'lib', 'stages.js'), "  const kept = decisions.length * agrees.length * union * (leanUnits ? confirms.length : 1);", "  const kept = decisions.length * agrees.length * union;",
    'theStageThreeCountIsTheLaunchsFoldWithoutTheSettings', 'the cost line says a third of the settings the launch builds'],
  ...
  [path.join(ROOT, 'lib', 'confirm.js'), "    pnl: kx * P(parts.c) + ux * P(parts.u) + P(parts.z),", "    pnl: P(parts.c) + P(parts.u) + P(parts.z),",
    'theMoneyAndSizeUnderTheMultipliers', 'sized never scales the money'],
  ...
  [path.join(ROOT, 'lib', 'coinsrun.js'), "    out[`${r.coin}|${r.geometry}`] = { band: r.band, yardstick: r.yardstick ?? null, rising: r.lean.rising || 0, falling: r.lean.falling || 0 };", "    out[`${r.coin}|${r.geometry}`] = { band: r.band, yardstick: r.yardstick ?? null, rising: 0, falling: 0 };",
  ...
  [path.join(ROOT, 'lib', 'stagework.js'), "  const ck = `${cellLabel}|${r.trade}|${r.ctx1 || ''}|${r.ctx2 || ''}|${r.geometry}|${confirm}`;", "  const ck = `${cellLabel}|${r.trade}|${r.ctx1 || ''}|${r.ctx2 || ''}|${r.geometry}`;",
  ...
  [path.join(ROOT, 'public', 'construct.js'), '  const stab = B_T3.includes(view.stab) ? (fold[3] ? view.stab : 3) : [1, 2, 3].includes(Number(view.stab)) ? Number(view.stab) : deepest;', '  const stab = deepest;',
