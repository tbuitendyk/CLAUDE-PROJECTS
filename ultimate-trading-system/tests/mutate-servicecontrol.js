...
const GUARDS = [
  ...
  [path.join(ROOT, 'lib', 'stagework.js'), '  if (r.agreed != null) { c.agr += r.agreed; c.agrN++; }', '',
  ...
  [path.join(ROOT, 'lib', 'stagework.js'), '  if (r.agreed != null) { k.agr += r.agreed; k.agrN++; }', '',
    'whatActuallyAgreedIsCarriedIntoBothTablesAndEveryRecord',
    'the every-coin table loses the column'],
  [path.join(ROOT, 'lib', 'stagework.js'), '      ...agreedFor(stream.decision, agr),', '',
    'whatActuallyAgreedIsCarriedIntoBothTablesAndEveryRecord',
    'no record carries what actually agreed, so the column exists on three screens with nothing behind it'],
