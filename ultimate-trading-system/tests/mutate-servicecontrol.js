...
const GUARDS = [
  ...
  [path.join(ROOT, 'lib', 'stages.js'), '  const copyOf = (d) => (rule.cut ? S4.nullCopy(all, rule, d) : S4.swapMoney(rows, d));',
    '  const copyOf = (d) => S4.swapMoney(rows, d);',
  ...
  // §16 -- the guided walk.
  [path.join(ROOT, 'lib', 'funnel.js'), '      const counts = cm.length > 0 && cm.every((v) => v != null && r.mean > v);',
    '      const counts = cm.length > 0 && cm.some((v) => v != null && r.mean > v);',
    'aValueCountsWhenItBeatsEveryCopyOrBothHalves',
    'a value counts by beating ONE scrambled copy instead of every one, and the recommendation is fitted to whichever copy was weakest'],
  [path.join(ROOT, 'lib', 'funnel.js'), '    const counts = a != null && b != null && ga != null && gb != null && a > ga && b > gb;',
    '    const counts = a != null && ga != null && a > ga;',
    'aValueCountsWhenItBeatsEveryCopyOrBothHalves',
