...
const GUARDS = [
  ...
  [path.join(ROOT, 'public', 'construct.js'), 'const fIsOpen = (st) => !!((st.cut && st.cut !== F_NEW) || fOpenOf(st.set));',
    ...
    'the screen is always Open, so there is no Home to go to and both presses are always live'],
  [path.join(ROOT, 'public', 'construct.js'), 'const fIsOpen = (st) => !!((st.cut && st.cut !== F_NEW) || fOpenOf(st.set));',
    'const fIsOpen = (st) => !!fOpenOf(st.set);',
    'theFunnelIsEitherHomeOrOpenAndThePressThatGoesHomeDropsTheWalk',
    'a Stage 4 record set on screen reads as Home, so the presses that act on it are ghosted'],
  [path.join(ROOT, 'public', 'construct.js'), "  fFreshWalk(st);\n  // AND PUT AWAY IS CLEARED WITH IT.", "  // AND PUT AWAY IS CLEARED WITH IT.",
    'theFunnelIsEitherHomeOrOpenAndThePressThatGoesHomeDropsTheWalk',
    'the press stops dropping the walk, so a part-built rule survives a press named for going home'],
  [path.join(ROOT, 'public', 'construct.js'), "  if (!confirm(`Drop this walk?", "  if (false && !confirm(`Drop this walk?",
    'theFunnelIsEitherHomeOrOpenAndThePressThatGoesHomeDropsTheWalk',
    'an hour of walking is thrown away on one press with no warning'],
  [path.join(ROOT, 'public', 'construct.js'), "  const noDrop = fHoldShown(st, away, open) ? ' disabled' : '';",
    "  const noDrop = open ? '' : ' disabled';",
    'theSetJustWrittenHasAPressBackToTheSteps',
    'the press is live with Worth walking? already on the screen — over a Stage 4 record set that was cut and finished, offering to drop a walk and naming a step nobody is on'],
  ...
  [path.join(ROOT, 'public', 'construct.js'), "    fMarkOpen(st.set, true);\n    fUnitChoose(st.set, key || 'all');", "    fUnitChoose(st.set, key || 'all');",
    'theFunnelIsEitherHomeOrOpenAndThePressThatGoesHomeDropsTheWalk',
    'choosing a coin and shape at Home leaves the screen at Home, so the boxes do nothing'],
  [path.join(ROOT, 'public', 'construct.js'), "      fMarkOpen(st.set, true);                            // it opens a walk (3.108.0)", "",
    'theFunnelIsEitherHomeOrOpenAndThePressThatGoesHomeDropsTheWalk',
    'Walk this one at Home opens nothing, so the press does nothing'],
  ...
  [path.join(ROOT, 'public', 'construct.js'), '  if (home) home.onclick = () => fGoFunnelHome(st, d);',
    '  if (home) home.onclick = () => fOpenNewRule(st, d);',
    'theSetJustWrittenHasAPressBackToTheSteps',
    'the press opens a walk instead of dropping one, so nothing can be abandoned'],
