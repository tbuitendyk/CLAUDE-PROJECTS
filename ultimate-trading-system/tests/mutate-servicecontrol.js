...
const GUARDS = [
  ...
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "async function makeKey(id) {\n  const rec = read(id);\n  mustBeOpen(rec, 'access');\n", "async function makeKey(id) {\n  const rec = read(id);\n",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "a key is made for a checklist whose step 2 is not open yet"],  // STEP 2, LET THE SYSTEM IN (3.265.0)
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "async function signIn(id) {\n  const rec = read(id);\n  mustBeOpen(rec, 'access');\n", "async function signIn(id) {\n  const rec = read(id);\n",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "the system signs in for a checklist whose step 2 is not open yet"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "'-o', 'StrictHostKeyChecking=accept-new'", "'-o', 'StrictHostKeyChecking=no'",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "the sign-in accepts a machine whose identity has changed"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "'-o', 'BatchMode=yes'", "'-o', 'BatchMode=no'",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "the sign-in can stop and wait for a password nobody will type"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "  rec.key = { how: 'made', publicKey, fingerprint:", "  rec.key = { how: 'made', privateKey: fs.readFileSync(path.join(dir, 'key'), 'utf8'), publicKey, fingerprint:",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "the private half is written into the checklist's record and every answer"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "  fs.chmodSync(dir, 0o700);", "  fs.chmodSync(dir, 0o755);",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "the key folder is readable by every account on the machine"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "  fs.chmodSync(kf, 0o600);", "  fs.chmodSync(kf, 0o644);",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "a pasted key file is readable by every account on the machine"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "  if (r.code !== 0 || !/^(ssh-|ecdsa-)/.test(r.stdout.trim())) {\n    fs.rmSync(kf, { force: true });\n", "  if (r.code !== 0 || !/^(ssh-|ecdsa-)/.test(r.stdout.trim())) {\n",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "a key file that cannot be used is kept anyway"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "    if (step.panel === 'access') dropKey(rec);", "    if (false) dropKey(rec);",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "a different way of signing in keeps the key made the other way"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "    if (rec.checks) for (const later of TEMPLATE.steps.slice(at)) delete rec.checks[later.id];", "    if (rec.checks) delete rec.checks[stepId];",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "a checklist moved to another machine still shows step 2 signed in to the old one"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "  if (before !== v && rec.checks) delete rec.checks[stepId];", "  if (false) delete rec.checks[stepId];",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "a changed address keeps the sign-in made to the old one"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "  if (/Permission denied/i.test(e)) return", "  if (false) return",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "a refused key is not said in words"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "  fs.rmSync(keyDirOf(id), { recursive: true, force: true });", "",
    "stepTwoLetsTheSystemInWithAKeyAndASignIn", "deleting a checklist leaves its key behind"],
