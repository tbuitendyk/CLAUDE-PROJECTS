...
const GUARDS = [
  ...
  // ---- TRAINING BY WHAT EACH WEEK WAS WORTH (3.69.0) ----
  [path.join(ROOT, 'lib', 'stagework.js'), "    return Math.max(0, m - trip) + m + trip;", "    return m;",
    ...
    'a still week is worth nothing, so nothing teaches the forecast to stand aside and every crumb is traded'],
  [path.join(ROOT, 'lib', 'stagework.js'), "  return stakes.map((x) => Math.min(cap, x / avg));", "  return stakes.map((x) => x / avg);",
    'theWeightOfATrainingWeekIsWhatItsDecisionWasWorth',
    'one freak week can outweigh fifty ordinary ones and the limit on the screen does nothing'],
  [path.join(ROOT, 'lib', 'stagework.js'), "  return stakes.map((x) => Math.min(cap, x / avg));", "  return stakes.map((x) => Math.min(cap, x));",
    'theWeightOfATrainingWeekIsWhatItsDecisionWasWorth',
    'the average week no longer counts 1, so the strength of the fit means something different from run to run'],
  [path.join(ROOT, 'lib', 'stagework.js'), "    const { model: m, chosenLambda } = await tuneAndTrain(Ztr, ytr, { onProgress: () => {}, exampleWeights: wAll });",
    ...
    'countingMoneyRatherThanWeeksChangesWhatBothForecastsLearn',
    'the first kind of forecast is trained with every week counting the same, whatever the owner asked for'],
  [path.join(ROOT, 'lib', 'stagework.js'), "    const m = await trainBoost(Xtr, ytr, { rounds: probe.bestRound, weights: wAll });",
    ...
    'countingMoneyRatherThanWeeksChangesWhatBothForecastsLearn',
    'the second kind of forecast is trained with every week counting the same, whatever the owner asked for'],
