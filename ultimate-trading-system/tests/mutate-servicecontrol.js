...
const GUARDS = [
  ...
  [path.join(ROOT, 'lib', 'stages.js'), '    if (sizing && c && c.sizing) mine.sizing = JSON.parse(JSON.stringify(c.sizing));', '    if (c && c.sizing) mine.sizing = JSON.parse(JSON.stringify(c.sizing));',
    'aSetSavedUnderANewNameCarriesWhatIsTickedAndTheLooksTheDataHasHad', 'a copy carries the sizing whether it is ticked or not'],
  [path.join(ROOT, 'lib', 'stages.js'), "    held: judgeSetsOf(src.id, 'held').length + (src.halflife || []).filter((r) => r && r.judge === 'hold').length + (Number(had.held) || 0),", "    held: judgeSetsOf(src.id, 'held').length + (src.halflife || []).filter((r) => r && r.judge === 'hold').length,",
    'aSetSavedUnderANewNameCarriesWhatIsTickedAndTheLooksTheDataHasHad', 'a copy of a copy forgets the held-back reads of the set the first was saved from'],
  [path.join(ROOT, 'lib', 'stages.js'), '    id: `${id}-v1`, at, release: ENGINE_VERSION, look: number + carried, stretch,', '    id: `${id}-v1`, at, release: ENGINE_VERSION, look: number, stretch,',
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "  if (carried) what.push(`${carried} held-back read(s) of ${doc.copiedFrom.name}, the set this was saved from, each a stamped look`);", '',
    'aSetSavedUnderANewNameCarriesWhatIsTickedAndTheLooksTheDataHasHad', "the looks line on Held says nothing of the reads the copy's data had"],
  ...
  [path.join(ROOT, 'public', 'construct.js'), "/copy`, { name, stops, sizing }, 'The scan target box on Tune lists the sets whose trades are captured.');", "/copy`, { name }, 'The scan target box on Tune lists the sets whose trades are captured.');",
