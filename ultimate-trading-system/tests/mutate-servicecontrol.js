...
const GUARDS = [
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "  if (doc.derived) return derivedRefusalOf(doc);\n  const parent = getSet((doc.parent || {}).id);\n  if (!parent) return 'the stage 3 set this was cut from is gone, so its records cannot be retrained';", "  if (doc.derived) return derivedRefusalOf(doc);\n  if (!unreadGateOf(doc)) return UNREAD_NO_PASS;\n  const parent = getSet((doc.parent || {}).id);\n  if (!parent) return 'the stage 3 set this was cut from is gone, so its records cannot be retrained';",
    'theTabsReadInProcessingOrderAndNoScreenBeforeVerifyAsksForAVerdict', 'History refuses to retrain until Verify, which comes after it, has stamped a verdict'],
  [path.join(ROOT, 'lib', 'stages.js'), "  const parent = getSet((doc.parent || {}).id);\n  if (!parent) return 'the stage 3 set this was cut from is gone, so its trades cannot be captured';", "  if (!gateOfSet(doc)) return UNREAD_NO_PASS;\n  const parent = getSet((doc.parent || {}).id);\n  if (!parent) return 'the stage 3 set this was cut from is gone, so its trades cannot be captured';",
  ...
  [path.join(ROOT, 'public', 'construct.js'), "  $('#view').innerHTML = hHalfLifePanelHtml(hSets, hChosen, hl);", "  $('#view').innerHTML = vGradePanelHtml(null) + hHalfLifePanelHtml(hSets, hChosen, hl);",
    'theTabsReadInProcessingOrderAndNoScreenBeforeVerifyAsksForAVerdict', 'the reserve grade panel is drawn on History again, before the verdict it needs exists'],
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "    return { layout, kind: 'after', intact: true, fromTs: at, chunks: null, why: null };", "    return { layout, kind: 'after', intact: false, fromTs: null, chunks: null, why: 'no' };",
    'theReserveGradeReadsTheUnreadWindowOffTheRecordWhateverTheLayout', 'a 70/15/15 set can never be graded on its unread window'],
  [path.join(ROOT, 'lib', 'stages.js'), "    return { layout, kind: 'after', intact: true, fromTs: at, chunks: null, why: null };", "    return { layout, kind: 'after', intact: true, fromTs: Number(hold.fromTs), chunks: null, why: null };",
    'theReserveGradeReadsTheUnreadWindowOffTheRecordWhateverTheLayout', 'the unread window of a 70/15/15 set begins where the held-back window begins, so the grade reads data the search touched'],
  [path.join(ROOT, 'lib', 'stages.js'), "  payload.unread = { fromTs: window.fromTs };", "  payload.unread = { fromTs: sealedOnUnitOf(doc).fromTs };",
    'theReserveGradeReadsTheUnreadWindowOffTheRecordWhateverTheLayout', 'the grade prices from the seal again, which a 70/15/15 set does not have'],
