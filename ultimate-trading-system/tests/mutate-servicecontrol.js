...
const GUARDS = [
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "\n    reserve: rec.reserve || null,\n    // and the actual date ranges this stage used", "\n    // and the actual date ranges this stage used",
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "  if (!kept.intact) {\n    return `${keptComplaint(kept, doc.name)} — a unit trained on other data would not be comparable `", "  if (false) {\n    return `${keptComplaint(kept, doc.name)} — a unit trained on other data would not be comparable `",
    "fillingInUnitsRefusesAnythingThatWouldNotBeComparable", "a unit trained on a damaged copy of the hours joins a set trained on the real ones, with nothing able to tell them apart"],
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "function canStartAgain(doc) { return doc.stage === 3 ? hasCheckpoint(doc.id) : planListed(doc); }", "function canStartAgain(doc) { return doc.stage === 3 ? false : planListed(doc); }",
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "        startsAgain: ['paused', 'interrupted', 'error'].includes(d.status) && canStartAgain(d),", '        startsAgain: false,',
  ...
  [path.join(ROOT, 'public', 'construct.js'), "  const paused = swPausedOptions(mine, selected);", "  const paused = '';",
  ...
  [path.join(ROOT, 'public', 'construct.js'), "${canContinue ? ` It can be started again from the stage ${doc.stage} section on Sweep.` : ''}", '',
  ...
  [path.join(ROOT, 'public', 'construct.js'), "  swStarting('again');\n", '',
  ...
  [path.join(ROOT, 'lib', 'stagework.js'), '    hours: p.hours || null, extras,', '    hours: null, extras,',
    'aRunPausedFromInsideAndStartedAgainEqualsTheReference', 'the kept hours never reach the loader, and a start-again reads whatever is on the box'],
  // ---- a child is stamped over every coin its parent was stamped over; a narrow pin is widened (3.84.1) ----
  [path.join(ROOT, 'lib', 'stages.js'), '  if (missing.length) {\n    const pc = checkKept(parent.hours);', '  if (false) {\n    const pc = checkKept(parent.hours);',
    'aUnitWhoseAgreementsWereLostGetsThemBackWithoutRepricingTheWholeUnit', 'a start-again of a set whose own record names one coin is refused for the coins it never named, instead of reading its parent\'s hours for them'],
  ...
  [path.join(ROOT, 'public', 'construct.js'), '    if (ny) { const list = glNotYetOf(gl4, picked); ny.innerHTML = glNotYetHtml(list, gl4.startsOn); ny.hidden = !list.length && !gl4.startsOn; }', '',
  ...
  [path.join(ROOT, "engine", "keystore.js"), "    if (process.platform !== 'win32' && (st.mode & 0o077) !== 0) throw new Error(", "    if (false) throw new Error(",
  ...
  [path.join(ROOT, "engine", "keystore.js"), "    return r ? { account, present: true, addedAt: r.addedAt, anyAddress:", "    return r ? { account, present: true, box: r.box, addedAt: r.addedAt, anyAddress:",
  ...
  [path.join(ROOT, "engine", "venues", "binance-account.js"), "  if (!r.ipRestrict && anyAddress !== true) refusals.push(", "  if (false) refusals.push(",
    "aKeyThatCanMoveMoneyOrIsNotLockedIsRefusedInWords", "a key open to any address is kept without the owner's tick"],
  ...
  [path.join(ROOT, "engine", "api.js"), "            if (v.checked && !v.ok) return { status: 400, json: { error: `the keys were not kept:", "            if (false) return { status: 400, json: { error: `the keys were not kept:",
  ...
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "  if (taken) bad(`there is already a setup for a platform called", "  if (false) bad(`there is already a setup for a platform called",
  ...
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "    if (step.writing) missing.push('this step is still being written');", "",
    "aStepStillBeingWrittenIsNeverDone", "a step still being written reads as done"],  // re-aimed 3.266.0: no step of template 2 is still being written; the test adds one
  ...
  [path.join(ROOT, "public", "setup.html"), "<span class=\"muted\">short name \u2014 letters, digits, dashes</span><input id=\"esShort\"", "<span class=\"muted\">record id</span><input id=\"esShort\"",
  ...
  [path.join(ROOT, "public", "setup.html"), "(c.clearsNote && ch[c.id] ? '<span class=\"note\">' + esc(c.clearsNote) + '</span>' : '')", "(c.clears && ch[c.id] ? '<span class=\"note\">changing this clears the ticks below: they were about the other machine</span>' : '')",
    "theChecklistDrawingTakesEveryWordFromTheTemplate", "a choice's note is typed in the page instead of taken from the template"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "{ id: 'binance', label: 'My exchange serves me there, and I may use it there' }", "{ id: 'binance', label: 'Binance serves me there, and I may use it there' }",
    "theTemplateHasStepOneInFullAndTheRestStillBeingWritten", "step 1 names one exchange as though it were the only one"],
  [path.join(ROOT, "engine", "lock.js"), "      d.setAAD(Buffer.from(aadFor(account)));\n", "",
    "theEngineTakesKeysAndNeverAnswersWithThem", "the locked keys open whatever account name they are sent under"],  // THE ENGINE CALLS OUT (3.266.0)
  [path.join(ROOT, "engine", "lock.js"), "    if (process.platform !== 'win32' && (fs.statSync(this.file).mode & 0o077) !== 0) {", "    if (false) {",
    "theLockIsTheEnginesAloneAndNeverLeaves", "a lock other users on the machine can read is used anyway"],
  [path.join(ROOT, "engine", "api.js"), "            if (b.apiKey !== undefined || b.secret !== undefined) return { status: 400,", "            if (false) return { status: 400,",
    "theEngineTakesKeysAndNeverAnswersWithThem", "the engine takes keys that arrive readable"],
  [path.join(ROOT, "engine", "venues", "binance-account.js"), "  return { ok: refusals.length === 0, refusals, tied: r.ipRestrict === true };", "  return { ok: refusals.length === 0, refusals, tied: true };",
    "aKeyThatCanMoveMoneyOrIsNotLockedIsRefusedInWords", "a key open to any address is said to be tied to one"],
  [path.join(ROOT, "engine", "keystore.js"), "anyAddress: anyAddress === true, tied:", "anyAddress: false, tied:",
    "theKeysAreStoredEncryptedAndNeverHandedBack", "the owner's address choice is not kept beside the keys"],
  [path.join(ROOT, "engine", "link.js"), "    fs.writeFileSync(t, JSON.stringify(rec), { mode: 0o600 });\n    fs.renameSync(t, this.tokenFile);", "    fs.writeFileSync(t, JSON.stringify(rec), { mode: 0o644 });\n    fs.renameSync(t, this.tokenFile);",
    "anEngineStartedWithItsCodeCallsInAndAnswersOverItsLink", "the engine's password is readable by every account on its machine"],
  [path.join(ROOT, "lib", "live", "enginehub.js"), "t.link === 'calls-out' && sameHex(t.tokenHash, h)", "t.link === 'calls-out'",
    "anEngineStartedWithItsCodeCallsInAndAnswersOverItsLink", "any password opens an engine's link"],
  [path.join(ROOT, "lib", "live", "enginehub.js"), "!sameHex(String(req.headers['x-uts-relay'] || ''), RELAY_SECRET)", "false",
    "aChildOfTheServiceReachesAnEngineThroughItsParent", "anyone on this machine can ask an engine anything through the relay"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "  if (rec.install.usedUtc) bad('that install code has been used already", "  if (false) bad('that install code has been used already",
    "stepTwoInstallsTheEngineWithAOneTimeCode", "an install code works more than once"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "  if (Date.parse(rec.install.expiresUtc) < now) bad('that install code has run out", "  if (false) bad('that install code has run out",
    "stepTwoInstallsTheEngineWithAOneTimeCode", "an install code works after its hour"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "  rec.install = { codeHash: sha256(code), madeUtc:", "  rec.install = { codeHash: sha256(code), code, madeUtc:",
    "stepTwoInstallsTheEngineWithAOneTimeCode", "the install code itself is kept on this machine"],
  [path.join(ROOT, "lib", "live", "targets.js"), "  if (!was || was.kind !== 'engine') { const e = new Error(`no platform called ${id || '(none)'}", "  if (was && was.kind !== 'engine') { const e = new Error(`no platform called ${id || '(none)'}",
    "theEnginesRecordIsTheOwnersAndRefusedInWordsWhenNotWhole", "an engine record is made by hand instead of by installing the engine"],
  [path.join(ROOT, "server.js"), "    if (b.remove !== true && (b.apiKey !== undefined || b.secret !== undefined)) return res.status(400)", "    if (false) return res.status(400)",
    "theKeysPassThroughThisMachineAndAreNeverKeptOrShown", "the service passes on keys that arrive readable"],
  [path.join(ROOT, "public", "setup.html"), "{ engine: enginePick(), locked, anyAddress: $('#takAny').checked }", "{ engine: enginePick(), locked, apiKey: key.value.trim(), anyAddress: $('#takAny').checked }",
    "theKeysPassThroughThisMachineAndAreNeverKeptOrShown", "the page sends the API key readable beside the locked pair"],
  [path.join(ROOT, "public", "setup.html"), "k.lockHere = await kl.fingerprint(k.lock.publicKey)", "k.lockHere = k.lock.fingerprint",
  ...
  [path.join(ROOT, "public", "setup.html"), "placeholder=\"none set\"></label>'\n    + '</div>'\n    + '<div class=\"row\" style=\"margin-top:.5rem\">'\n    + '<button data-setquota=", "placeholder=\"none set\"></label>'\n    + '<button data-setquota=",
    "theComputeTabGivesEveryButtonARowOfItsOwn", "Set the ceiling shares a row with its box again"],
  [path.join(ROOT, "public", "setup.html"), "+ '<span class=\"note\">' + (cSvcMsg['quota|' + u.unit] || '') + '</span>'", "+ ''",
    "theComputeTabGivesEveryButtonARowOfItsOwn", "what Set the ceiling answered is no longer written beside it"],
  [path.join(ROOT, "lib", "live", "targets.js"), "  const r = { ...was, name: String((rec || {}).name || '').trim(),", "  const r = { ...was, ...(rec || {}), name: String((rec || {}).name || '').trim(),",
    "theEnginesRecordIsTheOwnersAndRefusedInWordsWhenNotWhole", "the engine form writes how an engine is reached, or its token's fingerprint"],
  [path.join(ROOT, "lib", "live", "targets.js"), "  if (!LINKS.includes(x.link)) out.push('the link: a platform calls this system');", "",
    "theEnginesRecordIsTheOwnersAndRefusedInWordsWhenNotWhole", "an engine record reached some other way than calling in is accepted"],
  [path.join(ROOT, "lib", "live", "enginelink.js"), "  return require('./enginehub').call(target.id, method, p, body, timeoutMs);", "  return Promise.resolve({ ok: false, status: 0, why: 'no link' });",
    "anEngineOnThisMachineIsFollowedAndDrawnByTheOnePath", "questions to an engine no longer go down its link"],
  [path.join(ROOT, "lib", "live", "enginelink.js"), "    const h = require('./enginehub').status(this.target.id);", "    const h = { linked: true, since: null, why: null };",
    "anEngineOnThisMachineIsFollowedAndDrawnByTheOnePath", "the screens say an engine is linked when it is not"],
  [path.join(ROOT, "public", "setup.html"), "<h3 style=\"margin-top:0\">The trading platform</h3>", "<h3 style=\"margin-top:0\">The trading engine</h3>",
    "theTradingPlatformSectionIsNamedAsItsRowIs", "the section is named one thing and the row above it another"],
  // WINDOWS AND MAC WITH NOTHING INSTALLED FIRST, THE PLATFORM BY ITS NAME, THE GREENLIGHT NOTE, THE ACCOUNT CHECKLIST (3.268.0)
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "if (!choices[c.id] && !(c.engineSays && said[c.id])) missing.push(", "if (!choices[c.id]) missing.push(",
    "stepTwoInstallsTheEngineWithAOneTimeCode", "a platform set up before the question was asked is asked its operating system after the fact"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "function systemOf(setup, eng = engineOf(setup)) { return saidOf(eng).os || (setup.choices || {}).os || null; }", "function systemOf(setup, eng = engineOf(setup)) { return (setup.choices || {}).os || saidOf(eng).os || null; }",
    "theComputeTabDrawsStepTwo", "the install command is made for the system chosen, not the one the platform said it runs on"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "{ id: 'os', label: 'Its operating system', engineSays: 'platform', options:", "{ id: 'os', label: 'Its operating system', when: { where: 'local' }, engineSays: 'platform', options:",
    "theTemplateHasStepOneInFullAndTheRestStillBeingWritten", "a rented server is not asked its operating system, so a Windows server gets the Linux command"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "{ when: { os: 'linux' }, paras: ['Linux: Debian 12 or 13,", "{ paras: ['Linux: Debian 12 or 13,",
    "theTemplateHasStepOneInFullAndTheRestStillBeingWritten", "the ceiling on memory and processor is promised on Mac and Windows, where nothing sets it"],
  [path.join(ROOT, "lib", "live", "enginesetup.js"), "      title: 'Install the platform',", "      title: 'Install the engine',",
    "theTemplateHasStepOneInFullAndTheRestStillBeingWritten", "the checklist calls the platform an engine again"],
  [path.join(ROOT, "public", "setup.html"), "  if (system === 'windows') return 'Set-ExecutionPolicy Bypass -Scope Process -Force; [Net.ServicePointManager]::SecurityProtocol = [Net.ServicePointManager]::SecurityProtocol -bor [Net.SecurityProtocolType]::Tls12; & ([scriptblock]::Create((irm '", "  if (system === 'windows') return '& ([scriptblock]::Create((irm '",
    "theComputeTabDrawsStepTwo", "the Windows command no longer runs in a plain Windows PowerShell"],
  [path.join(ROOT, "public", "setup.html"), "linux: 'for Linux, in a terminal on the machine' }", "linux: 'for a terminal on the machine' }",
    "theComputeTabDrawsStepTwo", "the Linux command stops saying it is for Linux"],
  [path.join(ROOT, "public", "setup.html"), "+ ' Set up a trading platform</button></div>'", "+ ' Set up a trading engine</button></div>'",
    "theComputeTabOpensTheChecklistFromItsButton", "the button calls the platform an engine again"],
  [path.join(ROOT, "public", "setup.html"), "set up in The trading platform section below. Each trading setup names the one it runs on '", "chosen on each trading setup on the Trade tab. Nothing about it is set here. '",
    "theTradingPlatformSectionIsNamedAsItsRowIs", "the row says again that nothing about the platform is set on this tab"],
  [path.join(ROOT, "public", "setup.html"), "checklistStepsHtml('as', sel.setup, aTr.setupTemplate, null)", "''",
    "theAccountTabDrawsItWithThePlatformsChecklistCode", "the trading account checklist is not drawn, or drawn some other way"],
  [path.join(ROOT, "engine", "install", "windows.ps1"), "    if ($gotNode -ne $NodeSha256[$plat]) { throw 'the Node.js download did not match its fingerprint; nothing was installed' }\n", "",
    "theInstallScriptsCheckThePackageAndOpenNothing", "a Node.js download is run on Windows without its fingerprint checked"],
  [path.join(ROOT, "engine", "install", "windows.ps1"), "    ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }", "    ForEach-Object { }",
    "theInstallScriptsCheckThePackageAndOpenNothing", "a reinstall on Windows leaves the old program running beside the new one"],
  [path.join(ROOT, "engine", "install", "windows.ps1"), "could be read\n        return", "could be read\n        exit 0",
    "theInstallScriptsCheckThePackageAndOpenNothing", "the Windows window closes before the lock's fingerprint can be read"],
  [path.join(ROOT, "engine", "install", "mac.sh"), "  [ \"$GOT_NODE\" = \"$NODE_SHA256\" ] || { echo \"the Node.js download did not match its fingerprint; nothing was installed\"; exit 5; }\n", "",
    "theInstallScriptsCheckThePackageAndOpenNothing", "a Node.js download is run on a Mac without its fingerprint checked"],
  [path.join(ROOT, "lib", "live", "greenlight.js"), "  if (startsOn()) return [];\n", "",
    "onATradingPlatformTheGreenlightSaysPaperNowAndLiveWaits", "the Greenlight page tells a setup going to a platform the old order program's limits"],
  [path.join(ROOT, "public", "construct.js"), "  if (on) return `<b>it goes to the Trade tab</b>, and can be started on Paper Books there, on ${esc(on.name)}; Live Trading waits until the trading platforms can place real orders`;\n", "",
    "onATradingPlatformTheGreenlightSaysPaperNowAndLiveWaits", "the Greenlight page says nothing of where a setup on a platform can start"],
  [path.join(ROOT, "lib", "accountsetup.js"), "  } else if (a.setup) bad(`there is already a setup for the trading account ${name} \u2014 one per account`);\n", "  }\n",
    "aChecklistIsKeptOnItsAccountAndOpensStepByStep", "a second checklist wipes the first one on the same account"],
  [path.join(ROOT, "lib", "account.js"), ", ...(had.setup ? { setup: had.setup } : {}) };", " };",
    "aChecklistIsKeptOnItsAccountAndOpensStepByStep", "saving a trading account again throws its checklist away"],
  [path.join(ROOT, "lib", "accountsetup.js"), "  account.saveAccountSetup(id, null);\n  return { removed: id };", "  account.deleteTradingAccount(id);\n  return { removed: id };",
    "aChecklistIsKeptOnItsAccountAndOpensStepByStep", "taking a checklist away takes the trading account with it"],
  // 3.269.0: price files read as they were at launch; stages 1 and 2 pause and start again
  [path.join(ROOT, "lib", "stages.js"), "  doc.status = doc.cancelRequested ? (canStartAgain(doc) ? 'paused' : 'cancelled') : 'error';", "  doc.status = doc.cancelRequested ? (doc.stage === 3 && hasCheckpoint(doc.id) ? 'paused' : 'cancelled') : 'error';",
    "aStageOnePausedWhileGoingKeepsEveryUnitItWroteAndStartsAgain", "a paused stage 1 or 2 run reads cancelled and is never offered to be started again"],
  [path.join(ROOT, "lib", "stages.js"), "  if (doc.stage === 2) return Array.isArray(plan.carried) && plan.carried.length > 0;", "  if (doc.stage === 2) return false;",
    "theListRowSaysWhetherASetCanBeStartedAgain", "a paused stage 2 run is never offered to be started again"],
  [path.join(ROOT, "lib", "stages.js"), "    if (have.has(row.u) && out.dup == null) out.dup = row.u;\n", "",
    "aStageOneOrTwoStartAgainRefusesWhatItCannotResume", "a start-again adds to a store that already holds a unit twice"],
  [path.join(ROOT, "lib", "stages.js"), "      if ((!Array.isArray(rg) || rg[1] > held[k] || rg[1] <= rg[0]) && out.past == null) out.past = row.u;\n", "",
    "aStageOneOrTwoStartAgainRefusesWhatItCannotResume", "a start-again adds to a set whose records point at blocks that are not on disk"],
  [path.join(ROOT, "lib", "stages.js"), "function landS1Unit(doc, w, u, i, settled, t0, total) {\n  if (doc.cancelRequested) return;", "function landS1Unit(doc, w, u, i, settled, t0, total) {",
    "aStageOnePausedWhileGoingKeepsEveryUnitItWroteAndStartsAgain", "every unit a pause drops is written down as a failure"],
  [path.join(ROOT, "lib", "stages.js"), "  if (doc.stage === 2) return continueStage2(doc);\n", "",
    "aStageTwoSetPausedPartWayAndStartedAgainIsTheRunNeverPaused", "a paused stage 2 run cannot be started again"],
  [path.join(ROOT, "lib", "stages.js"), "    plan: { units: carried.length, carried: carried.map((row) => row.u) },", "    plan: { units: carried.length },",
    "aStageTwoSetPausedPartWayAndStartedAgainIsTheRunNeverPaused", "a stage 2 set does not record what it carried and can never be started again"],
  [path.join(ROOT, "lib", "stages.js"), "    perMember: res.perMember || null,\n    score: res.score, beat: res.beat, pairs: res.pairs, lead: res.lead,", "    score: res.score, beat: res.beat, pairs: res.pairs, lead: res.lead,",
    "aStageOneSetPausedPartWayAndStartedAgainIsTheRunNeverPaused", "the stage 1 record loses each member's own reading, as the old fill-in's did"],
  [path.join(ROOT, "lib", "stages.js"), "      if (ownsRuns && d.status === 'running' && (!activeSet || activeSet.id !== d.id)) {", "      if (d.status === 'running' && (!activeSet || activeSet.id !== d.id)) {",
    "aStrandedRunningSetIsMarkedInterruptedByTheServiceAlone", "any process that lists the sets marks the service's running set as broken off and rewrites its document"],
  [path.join(ROOT, "server.js"), "stages.ownRuns();\n", "",
    "aStrandedRunningSetIsMarkedInterruptedByTheServiceAlone", "the service never marks a set a restart left running"],
  [path.join(ROOT, "public", "construct.js"), "<button id=\"swStop\" class=\"danger\">Pause</button>", "<button id=\"swStop\" class=\"danger\">${row.stage === 3 ? 'Pause' : 'Stop'}</button>",
    "theSweepOffersAPausedRunWhereANewOneIsSetUp", "a stage 1 or 2 run is stopped by a button that says Stop"],
  [path.join(ROOT, "public", "construct.js"), "const swPausedRow = (x) => !!(x && x.startsAgain);", "const swPausedRow = (x) => !!(x && x.stage === 3 && x.startsAgain);",
    "theSweepOffersAPausedRunWhereANewOneIsSetUp", "a paused stage 1 or 2 run is not offered in its box"],
  [path.join(ROOT, "public", "construct.js"), "    const cont = swContinueOf(1);\n    if (cont) { await swStartAgain(1, cont); return; }\n", "",
    "theSweepOffersAPausedRunWhereANewOneIsSetUp", "Start stage 1 launches a new set instead of starting the paused one again"],
  [path.join(ROOT, "public", "construct.js"), "    const shut = !!swContinueOf(n);", "    const shut = n === 3 && !!swContinueOf(n);",
    "aPausedRunCanBeDeletedFromWhereItIsChosen", "a paused stage 1 or 2 run chosen leaves its boxes live"],
  [path.join(ROOT, "lib", "live", "view.js"), "link: mirror.linkStatus(), lastHealth: mirror.lastHealth,", "link: mirror.status, lastHealth: mirror.lastHealth,",
    "theTradeScreenAsksTheLinkWhetherThePlatformIsFollowed", "the Trade screen says following the platform for a platform that has gone"],
  [path.join(ROOT, "public", "setup.html"), "it is the name the platform is saved under when it is installed, at step 2", "it is the one the platform is saved under at step 6",
    "theShortNameNoteNamesAStepTheChecklistHas", "the note points at a step the checklist does not have"],
  // 3.270.0: each worker's share is held so workers x share stays under allowed
  [path.join(ROOT, "lib", "throttle.js"), "  return Math.max(0, Math.min(pct, Math.ceil(Number(allowed) / n) - 1));", "  return Math.max(0, Math.min(pct, Math.floor(Number(allowed) / n)));",
    "theHeldShareKeepsTheWorkersStrictlyUnderTheCeiling", "the workers together reach the ceiling exactly, and the machine pauses the whole service every tenth of a second"],
  [path.join(ROOT, "lib", "throttle.js"), "    return Math.round((quota / period) * 100);", "    return Math.round(quota / period);",
    "theCeilingIsReadFromTheMachinesOwnRecord", "the ceiling is read a hundred times too small, and every worker is parked"],
  [path.join(ROOT, "lib", "throttle.js"), "heldCache = { v: heldPct(askedCpuPct(), workersNow(), allowedPct()), at: Date.now() };", "heldCache = { v: askedCpuPct(), at: Date.now() };",
    "runningWorkHonoursTheHeldShare", "the Compute tab says each worker is held under allowed while running work goes on at the share as set"],
  [path.join(ROOT, "lib", "pool.js"), "{ workerData: { poolSize: size } }", "{ workerData: { poolSize: 1 } }",
    "aWorkerCountsThePoolItWasStartedIn", "every worker holds its share as though it were alone, and five of them overrun the ceiling"],
  [path.join(ROOT, "server.js"), "return { allowed, workers, pct: throttle.heldPct(throttle.askedCpuPct(), workers, allowed) };", "return { allowed, workers, pct: throttle.askedCpuPct() };",
    "theComputeTabSaysTheShareAsSetWhatIsHeldAndTheCeiling", "the Compute tab says nothing is held when the workers are being held"],
  [path.join(ROOT, "public", "setup.html"), "  else if (h.pct < c.pct) first =", "  else if (false) first =",
    "theComputeTabSaysTheShareAsSetWhatIsHeldAndTheCeiling", "the Compute tab says workers x share is under allowed while each worker is being held below the share the owner set"],
  // 3.271.0: stage 1 keeps each coin's hours at its launch, and every stage reads them
  [path.join(ROOT, "lib", "keephours.js"), "coins[sym] = keepCoin(sym, rowsOnDisk(sym, range));", "coins[sym] = keepCoin(sym, rowsOnDisk(sym, {}));",
    "startAndEndAreWhatIsKeptWhenAllLoadedDataIsNotTicked", "start and end on Sweep are ignored and a run reads every month the box holds"],
  [path.join(ROOT, "lib", "hours.js"), "  if (sha(text) !== entry.sha256) throw new Error(", "  if (false) throw new Error(",
    "aCopyIsSharedAndOneThatIsGoneOrDamagedRefusesByCoin", "a copy holding other hours is read as the hours the run was launched on"],
  [path.join(ROOT, "lib", "hours.js"), "  return { intact: !missing.length && !damaged.length, missing, damaged };", "  return { intact: true, missing, damaged };",
    "aCopyIsSharedAndOneThatIsGoneOrDamagedRefusesByCoin", "a set whose kept hours are gone or damaged is called whole, and its start-again fails unit by unit"],
  [path.join(ROOT, "lib", "bracketwork.js"), "  const loaded = kept\n    ? { rows: readCoin(sym, kept) }", "  const loaded = false\n    ? { rows: readCoin(sym, kept) }",
    "aUnitReadsTheCopyAndNeverTheBox", "a unit reads the box instead of the hours kept with its set, and newer data changes the run"],
  [path.join(ROOT, "lib", "bracketwork.js"), "  if (p.hours && !kept) throw new Error(", "  if (false && p.hours && !kept) throw new Error(",
    "aUnitReadsTheCopyAndNeverTheBox", "a coin the set never kept is read off the box, as it is today"],
  [path.join(ROOT, "lib", "stages.js"), "  doc.hours = keepHours(coinsOfUnits(units), p);", "  doc.hours = keepHours(coinsOfUnits(units).slice(0, 1), p);",
    "theEngineKeepsTheHoursAtTheLaunchAndHandsThemToEveryTask", "a stage 1 launch keeps one coin's hours and its units cannot read the others"],
  [path.join(ROOT, "lib", "stages.js"), "  return { at: h.at, digest: h.digest, coins: { ...h.coins } };", "  return { at: h.at, digest: h.digest, coins: {} };",
    "theEngineKeepsTheHoursAtTheLaunchAndHandsThemToEveryTask", "a child keeps none of its parent's hours and every one of its units is refused"],
  [path.join(ROOT, "lib", "stages.js"), "  if (h && h.coins && typeof h.coins === 'object' && Object.keys(h.coins).length) return h.coins;", "  return (h && h.coins) || null;",
    "anOlderSetIsBroughtForwardToExactlyTheHoursItsUnitsRead", "a set that keeps no hours is priced off whatever the box holds"],
  [path.join(ROOT, "lib", "stages.js"), "  const kept = checkKept(parent.hours);\n  if (!kept.intact) {\n    throw new Error(`${keptComplaint(kept, parent.name)} — a mismatch refuses, it never mixes`);", "  const kept = checkKept(parent.hours);\n  if (false) {\n    throw new Error(`${keptComplaint(kept, parent.name)} — a mismatch refuses, it never mixes`);",
    "theChainRefusalsNameThemselves", "a child is launched from a parent that keeps no hours, and every unit of it fails"],
  [path.join(ROOT, "lib", "stages.js"), "  const kept = checkKept(doc.hours);\n  if (!kept.intact) {\n    throw new Error(`${keptComplaint(kept, doc.name)} — the rest of this run would be priced on different prices", "  const kept = checkKept(doc.hours);\n  if (false) {\n    throw new Error(`${keptComplaint(kept, doc.name)} — the rest of this run would be priced on different prices",
    "aStartAgainRefusesWhatItCannotResume", "a stage 3 run whose kept hours are damaged is started again, and fails unit by unit"],
  [path.join(ROOT, "lib", "stages.js"), "  if (mine.length) removeUnused(mine, keptFilesInUse());", "  if (false) removeUnused(mine, keptFilesInUse());",
    "aCopyIsDeletedWithTheLastSetThatNamesIt", "a deleted set's kept hours stay on the box forever"],
  [path.join(ROOT, "lib", "keephours.js"), "    if (keep.has(f) || !FILE_RE.test(f)) continue;", "    if (!FILE_RE.test(f)) continue;",
    "aCopyIsDeletedWithTheLastSetThatNamesIt", "deleting a set deletes the hours its parent or child still reads"],
  [path.join(ROOT, "lib", "keephours-repair.js"), "    if (!buf || crypto.createHash('sha256').update(buf).digest('hex') !== x.sha256) {", "    if (!buf) {",
    "anOlderSetIsBroughtForwardToExactlyTheHoursItsUnitsRead", "an older set whose price files changed is brought forward to the changed hours, as though its units had read them"],
  [path.join(ROOT, "lib", "keephours-repair.js"), "    const buf = launchBytesOf(raw, x.bytes);", "    const buf = raw;",
    "anOlderSetIsBroughtForwardToExactlyTheHoursItsUnitsRead", "an older set launched on a day that grew since is called lost, and its work can never be priced again"],
  [path.join(ROOT, "public", "construct.js"), "${esc(sym)} ${hour(e.fromTs)} → ${hour(e.toTs)}", "${esc(sym)} ${hour(e.toTs)}",
    "theRunHeaderSaysEachCoinsFirstAndLastHour", "Boards says where each coin's hours end and not where they start"],
];
