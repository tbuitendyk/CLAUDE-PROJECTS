...
const GUARDS = [
  ...
  [path.join(ROOT, 'lib', 'stages.js'), '    const rows = applySort(3, tagged, doc.sort, (a, b) => a._i - b._i);', '    const rows = tagged;',
  ...
  [path.join(ROOT, 'lib', 'stages.js'), '  if (!records.some((r) => !r.ctx1)) delete out.quorumSingles;', '',
    'theDeclaredAgreementNamesOnlyTheBarsTheUnitsHold', 'a contexts-only run names a 6-member bar no unit holds a vote for'],
  [path.join(ROOT, 'lib', 'stages.js'), '  if (!records.some((r) => r.ctx1)) delete out.quorumContexts;', '',
    'theDeclaredAgreementNamesOnlyTheBarsTheUnitsHold', 'a singles-only run names the with-contexts bar again — the exact dishonesty the owner ordered out'],
  [path.join(ROOT, 'lib', 'stages.js'), '  const cell = cellForUnits(params.cell, parentRecords);', '  const cell = params.cell;',
    'theDeclaredAgreementNamesOnlyTheBarsTheUnitsHold', 'the launch declares bars its units cannot use and every setting name carries them'],
  [path.join(ROOT, 'lib', 'stages.js'), '      cell = cellForUnits(cell, records);', '',
  ...
  [path.join(ROOT, 'public', 'construct.js'), 'b3WireRankSort(doc, mount);', '',
  ...
  [path.join(ROOT, 'public', 'construct.js'), '      b3SaveView({ openS3: [...keys] });\n      b3RedrawPeggedToCoinHead();',
  ...
  [path.join(ROOT, 'public', 'construct.js'), "b3FilterGrid('S3R'", "b3FilterGridX('S3R'",
    'everyTableCarriesFiltersAFoldAndSortableColumns', 'the ranked table loses its filters'],
  [path.join(ROOT, 'public', 'construct.js'), 'id="s3AgreeRule"', 'id="s3AgreeRuleX"',
