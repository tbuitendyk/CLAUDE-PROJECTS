...
const GUARDS = [
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "  if (d && d.detail && Object.keys(d.detail).length) return stampFromEntries(id, d.detail);\n", "",
  ...
  [path.join(ROOT, "lib", "pin.js"), "  if (bytes < 2 || raw[bytes - 2] !== 0x7d || raw[bytes - 1] !== 0x2c) {", "  if (bytes < 2) {",
    "thePinnedLoaderReadsExactlyTheFilesTheStampLists", "a price file whose earlier hours moved is read cut at the launch's size, as though it were the launch's"],
  [path.join(ROOT, "lib", "pin.js"), "  if (raw.length < bytes) return { why: `it is shorter than the ${bytes} bytes it held at launch` };\n", "",
    "thePinnedLoaderReadsExactlyTheFilesTheStampLists", "a price file that lost hours is not said to be shorter"],
  [path.join(ROOT, "lib", "manifest.js"), "  if (!Number.isFinite(bytes) || st.size === bytes) return hashOfFile(file);\n", "  return hashOfFile(file);\n",
    "thePinnedLoaderReadsExactlyTheFilesTheStampLists", "a day file that only gained hours refuses every start-again and child launch, as it did to S1-ALL-20260926"],
  [path.join(ROOT, "lib", "pipeline.js"), "    const cut = launchBytesOf(fs.readFileSync(path.join(CACHE_DIR, f)), sizeAtLaunch.get(f));", "    const cut = { buf: fs.readFileSync(path.join(CACHE_DIR, f)) };",
    "thePinnedLoaderReadsExactlyTheFilesTheStampLists", "a run reads hours its launch never saw"],
  [path.join(ROOT, "lib", "stages.js"), "  if (d && d.detail && Object.keys(d.detail).length) return stampFromEntries(id, d.detail);\n", "",
    "aChildIsStampedOverEveryCoinItsParentWasStampedOver", "a child is stamped with its parent's files as they are now, and reads hours its parent never read"],
  ...
  [path.join(ROOT, "lib", "stages.js"), "    if (!d || d.status !== 'cancelled' || !planListed(d)) continue;", "    if (!d || d.status !== 'cancelled') continue;",
    "aStoppedStageOneSetReadsPausedAndNothingElseChanges", "a stage 1 set that cannot be started again is called paused"],
