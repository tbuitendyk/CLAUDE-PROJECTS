...
const GUARDS = [
  ...
  [path.join(ROOT, 'lib', 'coinsrun.js'), "  return coinsRecords().passers.rows.filter((r) => r.ticked).map((r) => ({ coin: r.coin, geometry: r.geometry }));", "  return coinsRecords().passers.rows.map((r) => ({ coin: r.coin, geometry: r.geometry }));",
  ...
  [path.join(ROOT, 'lib', 'coinsrun.js'), "    out[`${r.coin}|${r.geometry}`] = { band: r.band, rising: r.lean.rising || 0, falling: r.lean.falling || 0 };", "    out[`${r.coin}|${r.geometry}`] = { band: r.band, rising: 0, falling: 0 };",
