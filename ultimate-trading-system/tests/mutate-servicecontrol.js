...
const GUARDS = [
  ...
  [path.join(ROOT, 'public', 'construct.js'), 'const fHoldShown = (st, away) => !!away || !st.walking;',
    'const fHoldShown = () => true;',
    'theRankingSectionHidesOnUsingAWalkControlAndOnNothingElse',
    'the section stays up through a whole walk, which is the order it was built for'],
  [path.join(ROOT, 'public', 'construct.js'), 'const fHoldShown = (st, away) => !!away || !st.walking;',
    'const fHoldShown = (st) => !st.walking;',
    'theRankingSectionHidesOnUsingAWalkControlAndOnNothingElse',
    'put away no longer brings the section back, so the press leaves nothing on screen but the boxes'],
  ...
  [path.join(ROOT, 'public', 'construct.js'), "${away ? `<div class=\"panel\">${putAwayNote}</div>` : `<div id=\"fWalkBody\">",
    "${false ? `<div class=\"panel\">${putAwayNote}</div>` : `<div id=\"fWalkBody\">",
    'thePutAwayPressIsTheOneBoardsDrawsAndItLeavesTheTopTwoSectionsUp',
    'put away does not collapse the walk, so the press does nothing on the screen it was asked for'],
