...
const GUARDS = [
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "  if (passers && !passers.length) throw new Error('no coin and shape is ticked on Coins — tick some there, or untick \"only the coins and shapes ticked on Coins\"');", "  if (false) throw new Error('no coin and shape is ticked on Coins');",
  ...
  [path.join(ROOT, 'public', 'construct.js'), "  ${cPassersPanel(d && d.passers)}\n  ${!recs.length ?", "  ${!recs.length ?",
  ...
  [path.join(ROOT, 'lib', 'stages.js'), "    if (!heldStandingOf(doc)) return NO_HELD_PASS;\n  }\n  if (doc.derived) {\n    if (!getSet(doc.derived.from)) return 'the set this half-life set was built from is gone, so its retrained members cannot be found';\n    if (!readHalfLifeRun(doc.derived.from, doc.derived.run)) return HL_MEMBERS_MISSING;\n  }\n  const busy = verifyBusy();               // a sweep", "    if (false) return NO_HELD_PASS;\n  }\n  if (doc.derived) {\n    if (!getSet(doc.derived.from)) return 'the set this half-life set was built from is gone, so its retrained members cannot be found';\n    if (!readHalfLifeRun(doc.derived.from, doc.derived.run)) return HL_MEMBERS_MISSING;\n  }\n  const busy = verifyBusy();               // a sweep",
