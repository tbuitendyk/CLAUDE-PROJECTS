...
echo "cross-site state POST refused (CSRF): HTTP $(curl -sS -o /dev/null -w '%{http_code}' -m 6 -X POST -H 'Content-Type: application/json' -H 'Origin: https://evil.example' --data '{\"to\":\"live\"}' $B/api/live/setups/x/state)"
echo "(read-only)"
