...
# stability test DETACHED (24 trainings overruns the request window), writing to
# /tmp/trainextent.out. Read it with classifier-trainextent-read.sh.
...
# else disqualifies the setup from a live test.
set -uo pipefail
rm -f /tmp/trainextent.out /tmp/trainextent.done
nohup bash -c "cd /opt/general-classifier && node -e '
...
'
 > /tmp/trainextent.out 2>&1; touch /tmp/trainextent.done" >/dev/null 2>&1 &
echo "fired; poll with classifier-trainextent-read.sh"
