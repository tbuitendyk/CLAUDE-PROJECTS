...
print('the four null runs. KEY: each count = setups (of 136) whose fold-by-fold money')
print('beats the mean of the reference arms (median > 0 and more than half of folds).')
print('null counts = each null run vs the other three — the observed null-run variation.')
print(f'  real-vs-nulls count: {real_count}')
print(f'  null-vs-rest counts: {luck_counts}  (max {mx}, min {mn}, spread {mx-mn})')
print(f'  derived line       : max + spread = {bar}')
verdict = 'ABOVE the line — the lead sits outside the null runs\\' own variation; replication rules may be designed (still no selection)' if real_count > bar else 'NOT above the line — this committee design\\'s edge, if any, is too small for this instrument to certify; design work proceeds on that honest basis'
print(f'  VERDICT            : {verdict}')
print()
print('TABLE: top 15 real setups by paired median over the four null runs (reported,')
print('not selected). KEY: pair med = median per-fold (real minus null average) dollars')
print('per \$100 book; pos = folds where real beat the null average / paired folds.')
