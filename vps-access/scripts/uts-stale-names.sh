#!/usr/bin/env bash
# uts-stale-names.sh -- READ-ONLY. Which of the held setting names would today's
# code NOT write, and why? Rebuilds each setting's name from the fields its own
# records carry and compares it with the name stored beside them.
#
# Streams the record store one block at a time and keeps only one entry per
# setting, so it never holds the whole block enumeration in memory. Opens
# nothing for writing.
set -uo pipefail
cd /opt/ultimate-trading-system
node --max-old-space-size=900 -e '
const fs = require("fs");
...
for (const f of fs.readdirSync(dir).filter((x) => /^s3-.*\.json$/.test(x))) {
  ...
  const bySi = new Map();          // si -> one sample record, fields only
  for (let b = 0; b < nBlocks; b++) {
    ...
    if (b % 400 === 0) console.log("     block", b, " settings seen", bySi.size);
  }
  console.log("   distinct settings in the records:", bySi.size);

  let same = 0;
  const diffs = new Map();         // "what changed" -> {n, samples[]}
  ...
  for (const r of bySi.values()) {
    ...
    // name the difference in a way that groups
    let kind = "other";
  ...
  console.log("   names today's code writes the same way:", same);
  ...
  // and separately: held names whose records are gone, or records whose name
  // is not in the held list at all
  let notHeld = 0;
  for (const r of bySi.values()) if (!heldSet.has(r.label)) notHeld++;
