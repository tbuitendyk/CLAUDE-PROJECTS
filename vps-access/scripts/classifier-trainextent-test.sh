#!/usr/bin/env bash
# classifier-trainextent-test.sh -- READ-ONLY. The declared stability test.
#
# QUESTION: does a setup's behaviour depend on how much history it is trained
# on? XLM/directional's member call rate moved 50.8% -> 8.2% when ~325 recent
# periods were added to its training set. If money swings with training extent,
# the setup is not tradeable at any backtest value.
#
# CONSTRUCTION: the SCORED periods are identical for every extent (the last
# 15% of history, never trained on). Only the amount of history feeding the
# members varies, always ending immediately before the scored slice. So any
# difference is attributable to training extent alone.
#
# READING RULE, declared before this ran: STABLE means net money keeps its SIGN
# across all extents AND the call rate stays within a factor of two. Anything
# else disqualifies the setup from a Binance test regardless of its other
# numbers.
set -uo pipefail
node -e '
...
(async () => {
  ...
  for (const c of CASES) {
    ...
    const pre = [...trainChunks, ...testChunks];   // everything before the scored slice
    const views = bracketLib.comboViews(3, geo.featureHours / 24).views;
    const specs = specsFor(3, "slim");
    console.log("== " + c.coin + " " + c.decision + " (" + c.tag + ", " + c.cell.entry
      + " t" + c.cell.tHours + ", band " + bandPct.toFixed(2) + "%, scored on " + holdChunks.length + " held-back periods)");
    ...
    for (const e of EXTENTS) {
      ...
      console.log("   " + (e*100).toFixed(0).padStart(4) + "%" + String(n).padStart(13)
        + (100*cr).toFixed(1).padStart(9) + "%" + (q + "/" + holdChunks.length).padStart(11)
        + String(r.trades || 0).padStart(8) + ("$" + r.pnl.toFixed(2)).padStart(10));
    }
    const nets = rows.map((r) => r.net), crs = rows.map((r) => r.cr);
    const signStable = nets.every((n) => n > 0) || nets.every((n) => n < 0);
    const crRatio = Math.max(...crs) / Math.min(...crs.filter((x) => x > 0) .concat([1e-9]));
    console.log("   VERDICT: net sign " + (signStable ? "STABLE" : "FLIPS (" + nets.map((n)=>n>0?"+":"-").join("") + ")")
      + " | call-rate spread x" + crRatio.toFixed(1) + " (limit x2)"
      + " -> " + ((signStable && crRatio <= 2) ? "STABLE" : "UNSTABLE — disqualified from a live test"));
    console.log("");
  }
