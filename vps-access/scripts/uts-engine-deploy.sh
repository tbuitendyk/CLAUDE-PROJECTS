...
echo "== 3. waiting for it to call this system again =="
ok=0
...
if [ $ok != 1 ]; then "${SSH[@]}" 'sudo cat /var/lib/uts-engine/link-status.json 2>/dev/null; sudo journalctl -u uts-engine --no-pager -n 15 | cut -c1-200' | sed 's/^/  /'; fi
