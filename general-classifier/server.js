...
// ---- THE OWNER'S RULE REGISTRY (owner directive, 2026-08-18) ---------------
// These five routes are the ONLY way an entry enters, changes or leaves the
// registry. Nothing in the engine writes to it; nothing seeds it. A rule the
// owner did not put here does not exist, which is the entire requirement:
// "ABSOLUTELY NOTHING YOU PUT INTO LISTS IS ALLOWED TO PERSIST."
//
// The list starts EMPTY and stays empty until the owner fills it. That is not
// an oversight to be smoothed over with defaults — a default is me choosing
// again.
const rules = require('./lib/rules');

app.get('/api/rules', (req, res) => {
  try {
    const all = rules.readRegistry().rules.sort((a, b) => (a.order || 0) - (b.order || 0));
    res.json({ rules: all, active: rules.listActive().length, total: all.length });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.post('/api/rules', csrfGuard, (req, res) => {
  try {
    ...
    const entry = {
      ...
      createdBy: 'owner',
      order: doc.rules.length,
    };
    const errors = rules.validateEntry(entry, { existing: doc.rules });
    if (errors.length) return res.status(400).json({ error: errors.join('; '), errors });
    doc.rules.push(entry);
    rules.writeRegistry(doc);
    res.json({ ok: true, rule: entry });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// Rename and reorder only. The MECHANICS of a rule are deliberately not
// editable in place: a rule that silently changes what it trades while keeping
// its id is how a live record stops meaning what it says. Change the mechanics
// by retiring this rule and adding the new one, so the history shows both.
app.patch('/api/rules/:id', csrfGuard, (req, res) => {
  try {
    const doc = rules.readRegistry();
    const r = doc.rules.find((x) => x.id === String(req.params.id));
    if (!r) return res.status(404).json({ error: `no rule ${req.params.id}` });
    const body = req.body || {};
    if (body.label != null) {
    ...
    if (body.order != null) {
      ...
      if (!Number.isInteger(n) || n < 0) return res.status(400).json({ error: 'order: must be a non-negative integer' });
      r.order = n;
    }
    if (body.config != null || body.provenance != null) {
      return res.status(400).json({
        error: 'mechanics and provenance are not editable in place — retire this rule and add the replacement, so the record shows both',
      });
    }
    rules.writeRegistry(doc);
...
// RETIRE keeps the row and its history; DELETE removes it outright. Both are
// the owner's call. Retire is offered first because erasing what once ran
// destroys the record, but the choice is not mine to withhold.
app.post('/api/rules/:id/retire', csrfGuard, (req, res) => {
  try {
    const doc = rules.readRegistry();
    const r = doc.rules.find((x) => x.id === String(req.params.id));
    if (!r) return res.status(404).json({ error: `no rule ${req.params.id}` });
    r.retiredUtc = new Date().toISOString();
    r.retiredReason = String((req.body || {}).reason || '').slice(0, 300) || null;
    rules.writeRegistry(doc);
    res.json({ ok: true, rule: r });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.delete('/api/rules/:id', csrfGuard, (req, res) => {
  try {
    const doc = rules.readRegistry();
    const before = doc.rules.length;
    doc.rules = doc.rules.filter((x) => x.id !== String(req.params.id));
    if (doc.rules.length === before) return res.status(404).json({ error: `no rule ${req.params.id}` });
    rules.writeRegistry(doc);
    res.json({ ok: true, removed: String(req.params.id) });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
